-- Attendance was counted three different ways: the result calculator credited
-- only 'present', the leaderboard credited 'present' or 'late', and the
-- attendance screen grouped 'late' with 'excused'. The same learner had
-- different attendance percentages depending on the screen.
--
-- One policy now applies, mirroring src/lib/attendance/policy.ts:
--   present, late  -> attended (a late learner was still taught)
--   excused        -> removed from the denominator, not counted against them
--   absent         -> missed
--
-- Only the attendance branch changes; everything else is as migration
-- 20260929000004 left it.
  ...
  for i in 1..array_length(keys,1) loop
    ...
    if k='theory' then
      ...
      if n_sessions>0 then
        -- present and late both count as taught; excused leaves the denominator.
        select
        ...
        -- Every session excused means nothing to measure, not a zero.
