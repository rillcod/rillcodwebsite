-- ============================================================================
-- Per-Teacher Personal Reports & Withdrawn Student Tracking
-- ============================================================================
-- 1. Adds 'withdrawn' flag for students marked as withdrawn/ended on roster or profile
-- 2. Upgrades teacher analytics to provide a complete Teacher Personal Report:
--    - Teacher Name
...
-- ---------------------------------------------------------------------------
-- 1. People Materialised View with Withdrawn Tracking
-- ---------------------------------------------------------------------------
...
CREATE MATERIALIZED VIEW public.accountability_people_mv AS
  ...
  SELECT
    ...
    ARRAY_REMOVE(ARRAY[
      -- Withdrawn student
      CASE WHEN pu.role = 'student' AND (ro.status IN ('withdrawn', 'ended', 'removed') OR lower(COALESCE(pu.enrollment_type, '')) = 'withdrawn')
           THEN 'withdrawn'          END,
...
-- ---------------------------------------------------------------------------
-- 2. Coverage Materialised View with Teacher Personal Reports & Withdrawn Totals
-- ---------------------------------------------------------------------------
