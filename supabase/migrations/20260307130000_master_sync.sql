-- =============================================
-- FINAL SYNC: Timetables, Reports, Lessons, and Prospective Students
-- =============================================

-- ─── 1. Timetables (multi-school) ────────────────────────────────
...
-- Timetable Policies: Fixes infinite recursion
DROP POLICY IF EXISTS "admin_all_timetables" ON public.timetables;
CREATE POLICY "admin_all_timetables" ON public.timetables 
  FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "admin_all_timetable_slots" ON public.timetable_slots;
CREATE POLICY "admin_all_timetable_slots" ON public.timetable_slots 
  FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "school_read_timetables" ON public.timetables;
CREATE POLICY "school_read_timetables" ON public.timetables 
  FOR SELECT TO authenticated USING (school_id IN (SELECT school_id FROM public.portal_users WHERE id = auth.uid() AND role = 'school'));
...
CREATE POLICY "school_read_slots" ON public.timetable_slots 
  FOR SELECT TO authenticated USING (timetable_id IN (SELECT t.id FROM public.timetables t JOIN public.portal_users p ON p.school_id = t.school_id WHERE p.id = auth.uid() AND p.role = 'school'));

DROP POLICY IF EXISTS "teacher_read_slots" ON public.timetable_slots;
CREATE POLICY "teacher_read_slots" ON public.timetable_slots 
...
CREATE POLICY "teacher_read_timetables" ON public.timetables 
  ...
  USING (
    ...
    OR id IN (SELECT timetable_id FROM public.timetable_slots WHERE teacher_id = auth.uid())
  );

DROP POLICY IF EXISTS "student_read_timetables" ON public.timetables;
CREATE POLICY "student_read_timetables" ON public.timetables 
  FOR SELECT TO authenticated USING (school_id IN (SELECT school_id FROM public.portal_users WHERE id = auth.uid() AND role = 'student'));
...
CREATE POLICY "student_read_slots" ON public.timetable_slots 
  FOR SELECT TO authenticated USING (timetable_id IN (SELECT t.id FROM public.timetables t JOIN public.portal_users p ON p.school_id = t.school_id WHERE p.id = auth.uid() AND p.role = 'student'));

-- ─── 2. Reports (student_progress_reports + settings) ───────────
...
CREATE POLICY "Teachers manage progress reports" ON public.student_progress_reports
  FOR ALL USING (
    teacher_id = auth.uid()
    OR EXISTS (SELECT 1 FROM public.portal_users pu WHERE pu.id = auth.uid() AND pu.role = 'admin')
  );
...
CREATE POLICY "Schools view own student reports" ON public.student_progress_reports
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE pu.id = auth.uid() AND pu.role = 'school' AND pu.school_id = student_progress_reports.school_id
    )
  );
...
CREATE POLICY "staff_read_plans" ON public.lesson_plans
  FOR SELECT USING (EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role IN ('admin','teacher')));

DROP POLICY IF EXISTS "staff_write_plans" ON public.lesson_plans;
CREATE POLICY "staff_write_plans" ON public.lesson_plans
  FOR ALL USING (EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role IN ('admin','teacher')));
...
CREATE POLICY "read_public_materials" ON public.lesson_materials
  FOR SELECT USING (is_public = TRUE OR EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role IN ('admin','teacher')));
...
CREATE TABLE IF NOT EXISTS public.prospective_students (
  ...
  email               TEXT NOT NULL,
  age                 INTEGER,
  gender              TEXT, grade TEXT, school_name TEXT, 
  school_id           UUID REFERENCES public.schools(id) ON DELETE SET NULL,
  parent_name TEXT, parent_email TEXT, parent_phone TEXT, course_interest TEXT, 
  preferred_schedule TEXT, hear_about_us TEXT, notes TEXT, status TEXT NOT NULL DEFAULT 'pending',
  is_active BOOLEAN NOT NULL DEFAULT FALSE, is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE POLICY "Staff can manage prospective students" ON public.prospective_students FOR ALL USING (EXISTS (SELECT 1 FROM public.portal_users WHERE id = auth.uid() AND role IN ('admin','teacher')));

-- ─── 4. Extra Columns ───────────────────────────────────────────

ALTER TABLE public.portal_users ADD COLUMN IF NOT EXISTS section_class text;
ALTER TABLE public.portal_users ADD COLUMN IF NOT EXISTS current_module text;
ALTER TABLE public.portal_users ADD COLUMN IF NOT EXISTS date_of_birth  date;
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS section text;
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES public.portal_users(id);
...
GRANT ALL ON TABLE public.timetable_slots TO authenticated, service_role;
