-- Automatic results were losing marks that no learner could ever earn.
--
-- 1. Category predicates disagreed with the data. "assignments" excluded
--    classwork, homework, project and practical, but every assignment in the
--    system is homework or project, so the category matched nothing at all.
--    Homework is now assignment work, and classwork keeps in-class exercises.
--
-- 2. A component with no evidence scored zero and still consumed its weight.
--    With the active scheme that made assignments (20) and assessment (15)
--    permanently unreachable: a learner scoring full marks on everything they
--    could be assessed on topped out at 65, so no automatic distinction was
--    possible. Weight is now spread across the components a class is actually
--    assessed on. Not attempting work that does exist still scores zero — the
--    distinction is between "never set" and "not done".
--
-- 3. Assessments were averaged equally. Each may now carry
--    metadata.weight (default 1) and the category average respects it.
...
begin
  ...
  for i in 1..array_length(keys,1) loop
    ...
    if k='theory' then
      ...
      -- Homework is assignment work. Only project and practical belong to the
      -- practical category, and classwork has its own.
      select exists(select 1 from public.assignments a
    ...
    -- Nothing of this kind was ever set for the class: it cannot be earned, so
    -- it must not dilute the result either.
    if not available or wt=0 then
      if wt>0 then not_assessed:=not_assessed||jsonb_build_array(k); end if;
    else
      raw:=coalesce(raw,0);              -- set but not attempted scores zero
    ...
    values(r.id,k,lbl,wt,raw,weighted,n,ids,
      case when k='attendance' then jsonb_build_object('held_sessions',n_sessions,'present_sessions',n_present)
           else jsonb_build_object('assessed',available) end,
      not available or n=0);
  end loop;

  -- Spread the scheme across what the class is actually assessed on.
...
comment on function public.recalculate_academic_result(uuid,uuid) is
  'Weights each component by the active scheme over the components the class is actually assessed on, respecting per-assessment metadata.weight. A component never set for the class is excluded; one that exists but was not attempted scores zero.';
