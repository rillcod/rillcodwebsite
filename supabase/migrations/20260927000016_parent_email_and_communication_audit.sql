-- ============================================================================
-- Parent Email Tracking & Communication Dispatch Analytics
-- ============================================================================
-- 1. Audits Parent Email Reachability & Parent-Student Matching:
--    - Parent Email matching (direct vs linked portal account)
--    - Parent Phone matching
--    - Fully matched vs Completely unmatched students
-- 2. Audits Parent Email Dispatches by Type / Template Key:
--    - Result Notifications (result_published / email_events)
--    - Parent Account Claims (parent_claim / onboarding)
--    - Billing & Fee Notices
--    - General Broadcasts & System Alerts
--    - Detailed counts for Sent, Delivered, Bounced/Failed, and Pending
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 1. People Materialised View with Parent Email & Phone Reachability
-- ---------------------------------------------------------------------------
  ...
  SELECT
    ...
    ARRAY_REMOVE(ARRAY[
      ...
      CASE WHEN pu.role = 'student'
           ...
           THEN 'no_class'           END,
      -- Active student with no report for current term
      CASE WHEN pu.role = 'student'
            AND COALESCE(pu.is_active, false)
            AND COALESCE(ro.status, 'active') NOT IN ('withdrawn', 'ended', 'removed')
            AND lower(COALESCE(pu.enrollment_type, '')) <> 'withdrawn'
      ...
      -- Active student with no parent phone
      CASE WHEN pu.role = 'student'
            AND COALESCE(pu.is_active, false)
            AND COALESCE(ro.status, 'active') NOT IN ('withdrawn', 'ended', 'removed')
            AND lower(COALESCE(pu.enrollment_type, '')) <> 'withdrawn'
      ...
      -- Active student with no parent email
      CASE WHEN pu.role = 'student'
            AND COALESCE(pu.is_active, false)
            AND COALESCE(ro.status, 'active') NOT IN ('withdrawn', 'ended', 'removed')
            AND lower(COALESCE(pu.enrollment_type, '')) <> 'withdrawn'
...
-- ---------------------------------------------------------------------------
-- 2. Coverage Materialised View with Parent Email & Communication Audit
-- ---------------------------------------------------------------------------
    ...
    jsonb_build_object(
      ...
      'by_email_type', COALESCE((
        ...
        FROM (
          ...
          GROUP BY COALESCE(l.template_key, 'result_notification')
        ) e), '[]'::jsonb),
