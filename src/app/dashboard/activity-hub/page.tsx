...
const SocialHub = dynamic(
  () =>
    import("../engage/CommunityPage").then((m) => ({
      default: m.CommunityPage,
    })),
  {
    loading: () => (
      <div className="p-12 text-center animate-pulse text-muted-foreground uppercase font-black tracking-widest">
        Loading Social Community...
      </div>
    ),
  }
);
const ResourceVault = dynamic(
  () => import("../vault/VaultPage").then((m) => ({ default: m.VaultPage })),
  {
    loading: () => (
      <div className="p-12 text-center animate-pulse text-muted-foreground uppercase font-black tracking-widest">
        Loading Code Vault...
...
const SkillQuests = dynamic(
  () =>
    import("../missions/MissionsPage").then((m) => ({
      default: m.MissionsPage,
    })),
  {
    loading: () => (
      <div className="p-12 text-center animate-pulse text-muted-foreground uppercase font-black tracking-widest">
        Loading Skill Quests...
      </div>
    ),
  }
);
const MasteryProtocol = dynamic(
  () =>
    import("../protocol/ProtocolPage").then((m) => ({
      default: m.ProtocolPage,
    })),
  {
    loading: () => (
      <div className="p-12 text-center animate-pulse text-muted-foreground uppercase font-black tracking-widest">
        Loading Mastery Protocol...
      </div>
    ),
  }
);
...
const TABS: TabConfig[] = [
  {
    ...
    bannerDescription:
      "Post updates, react to peer builds, and give feedback in the community feed. Collaborate on projects, celebrate wins, and grow together as a coding community.",
    icon: UserGroupIcon,
    color: "text-primary",
  },
  {
    ...
    bannerDescription:
      "Save, tag, and search your go-to code snippets, templates, and references. Your private library stays organised and instantly accessible — no more hunting through old projects.",
    icon: ArchiveBoxIcon,
    color: "text-primary",
  },
  {
  ...
  },
  {
    ...
    bannerDescription:
      "Follow a structured track from fundamentals to advanced concepts. Complete milestones, earn certifications, and build the disciplined habits that separate great developers from the rest.",
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-30 dark:opacity-20">
        <div
          className={`absolute top-0 right-0 w-[500px] h-[500px] ${currentAesthetic.bg20} blur-[120px] transition-all duration-1000`}
        />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl sm:text-6xl font-black tracking-tighter uppercase italic leading-none">
              ...
              <br />
              <span
                className={`text-transparent bg-clip-text bg-gradient-to-r ${currentAesthetic.textGradient} transition-all duration-700`}
              >
                Community.
              </span>
            </h1>
              ...
              return (
                ...
                >
                  {isSelected && (
                    <motion.div
                      layoutId="activeBG"
                      className={`absolute inset-0 ${aes.bg5}`}
                    />
                  )}
                  <tab.icon
                    className={`w-5 h-5 relative z-10 ${
                      isSelected ? tab.color : "text-muted-foreground/40"
                    }`}
                  />
                  <div className="relative z-10">
                    ...
                    <div
                      className={`h-0.5 mt-2 transition-all duration-500 ${
                        isSelected ? `w-full ${aes.bgFull}` : "w-4 bg-border"
                      }`}
                    />
                  </div>
                </button>
        ...
        <div className="bg-card border border-border overflow-hidden relative group">
          <div
            className={`absolute top-0 right-0 w-[400px] h-[400px] ${currentAesthetic.bg10} blur-[100px] -mr-32 -mt-32 pointer-events-none`}
          />
          <div className="p-8 sm:p-12 relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-8">
            <div className="space-y-4 max-w-xl">
              <h2
                className={`text-2xl sm:text-3xl font-black uppercase tracking-tight ${currentTab.color}`}
              >
                {currentTab.name}
              </h2>
            ...
            <div
              className={`w-20 h-20 ${currentAesthetic.bg20} border ${currentAesthetic.border30} flex items-center justify-center`}
            >
        ...
        <div className="min-h-[600px] bg-card/30 border border-border relative overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "protocol" && (
                <MasteryProtocol isEmbedded={true} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
