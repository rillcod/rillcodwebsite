...
export default function TransferRequestCenterPage() {
  ...
  return <div className={`mx-auto max-w-6xl space-y-6 ${MOBILE_PAGE_ROOT}`}>
    ...
    {message && <div className={`flex gap-2 rounded-xl border p-4 text-sm ${message.ok ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-rose-500/30 bg-rose-500/10 text-rose-600 dark:text-rose-400'}`}>{message.ok ? <CheckCircleIcon className="h-5 w-5 shrink-0" /> : <ExclamationTriangleIcon className="h-5 w-5 shrink-0" />}<span>{message.text}</span></div>}
