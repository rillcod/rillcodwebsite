  ...
  return <div className="mx-auto max-w-6xl space-y-6 pb-24">
    ...
    {tab === 'request' ? <div className="space-y-5">
      ...
      {destinationId && <div className="overflow-hidden rounded-2xl border border-border bg-card">
        ...
        {selected.size > 0 && <div className="space-y-3 border-t border-border p-4"><textarea rows={3} value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason for the selected transfers (minimum 10 characters)" className="w-full rounded-xl border border-border bg-background p-3 text-sm text-foreground" /><div className="flex items-center justify-between gap-3"><span className="text-xs text-muted-foreground">{selected.size} selected - {reason.trim().length}/10 characters</span><button disabled={busy || reason.trim().length < 10} onClick={sendSelected} className="rounded-xl bg-primary px-5 py-2.5 text-sm font-black text-primary-foreground disabled:opacity-40">Send {selected.size} request{selected.size === 1 ? '' : 's'}</button></div></div>}
      </div>}
      {outgoing.length > 0 && <div className="rounded-2xl border border-border bg-card p-5"><h2 className="text-sm font-black text-foreground">Awaiting approval ({outgoing.length})</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{outgoing.map(request => <div key={request.id} className="rounded-xl border border-border bg-background p-3"><p className="text-sm font-bold text-foreground">{request.student?.full_name}</p><p className="mt-1 text-xs text-muted-foreground">{request.from_teacher?.full_name}: {request.from_class?.name} to {request.to_class?.name}</p></div>)}</div></div>}
    </div> : <div className="overflow-hidden rounded-2xl border border-border bg-card">
      <div className="sticky top-0 z-10 flex flex-wrap items-center gap-3 border-b border-border bg-card/95 p-4 backdrop-blur"><div className="flex-1"><h2 className="text-sm font-black text-foreground">Incoming requests ({incoming.length})</h2><p className="text-xs text-muted-foreground">Select only the students you are ready to release.</p></div>{incoming.length > 0 && <><button onClick={() => setSelectedIncoming(allIncomingSelected ? new Set() : new Set(incoming.map(row => row.id)))} className="rounded-xl border border-border px-3 py-2 text-xs font-black text-foreground">{allIncomingSelected ? 'Clear all' : 'Select all'}</button><button disabled={busy || selectedIncoming.size === 0} onClick={() => decideSelected('decline')} className="rounded-xl border border-rose-500/30 px-3 py-2 text-xs font-black text-rose-400 disabled:opacity-40">Decline selected</button><button disabled={busy || selectedIncoming.size === 0} onClick={() => decideSelected('approve')} className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-black text-white disabled:opacity-40">Approve & move ({selectedIncoming.size})</button></>}</div>
      <div className="divide-y divide-border">{incoming.length ? incoming.map(request => { const checked = selectedIncoming.has(request.id); return <button key={request.id} onClick={() => toggle(selectedIncoming, setSelectedIncoming, request.id)} className={`flex w-full gap-3 p-4 text-left ${checked ? 'bg-emerald-500/10' : 'hover:bg-muted/40'}`}><span className={`mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded border ${checked ? 'border-emerald-500 bg-emerald-600' : 'border-border'}`}>{checked && <CheckCircleIcon className="h-4 w-4 text-white" />}</span><span className="min-w-0 flex-1"><span className="block text-sm font-black text-foreground">{request.student?.full_name}</span><span className="mt-1 block text-xs text-muted-foreground">{request.requester?.full_name} requests {request.from_class?.name} to {request.to_class?.name}</span><span className="mt-2 block rounded-lg bg-muted px-3 py-2 text-xs text-foreground">{request.reason}</span></span></button>; }) : <div className="p-16 text-center"><CheckCircleIcon className="mx-auto h-10 w-10 text-emerald-500/50" /><p className="mt-3 text-sm font-bold text-foreground">All caught up</p><p className="text-xs text-muted-foreground">No transfer requests are waiting for you.</p></div>}</div>
    </div>}
  </div>;
}