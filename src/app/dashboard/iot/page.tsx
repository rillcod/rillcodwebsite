  ...
  if (dbLoading) return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-muted-foreground text-sm">Connecting to IoT infrastructure…</p>
      </div>
    </div>
  );
            ...
            <div className="divide-y divide-white/5">
              {devices.map(d => (
                <div key={d.id}>
                  <div
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">{d.location} · {d.ip}</p>
                        {d.status === 'online' && (
                      ...
                      <div className="flex-shrink-0 text-right">
                        <p className="text-xs text-muted-foreground">{d.uptime > 0 ? `${d.uptime.toFixed(1)}%` : 'Offline'}</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">{ago(d.lastSeen)}</p>
                        {expanded === d.id
                          ? <ChevronUpIcon className="w-4 h-4 text-muted-foreground mt-1 ml-auto" />
                          : <ChevronDownIcon className="w-4 h-4 text-muted-foreground mt-1 ml-auto" />
                        }
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Expanded detail */}
                  {expanded === d.id && (
                    <div className="px-5 pb-5 bg-white/[0.02] border-t border-border">
          ...
          <div className="flex items-start gap-3 bg-card shadow-sm border border-border rounded-none px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Create an <code className="bg-muted px-1 rounded">iot_devices</code> table in Supabase with columns:
                {' '}<code className="bg-muted px-1 rounded">id, name, type, status, location, ip_address, last_seen, uptime_pct, temperature, cpu_usage, memory_usage, network_usage, battery_pct</code>.
