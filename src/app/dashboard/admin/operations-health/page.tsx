...
type HealthRow = {
  job_name: string; expected_interval_minutes: number; last_finished_at: string | null;
  last_success_at: string | null; next_expected_at: string | null; last_status_code: number | null;
  last_duration_ms: number | null; last_error: string | null; consecutive_failures: number;
};
type DeadLetter = {
  id: string; source: string; job_type: string; user_id: string | null; error: string;
...
export default function OperationsHealthPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setHealth(json.health ?? []); setDeadLetters(json.deadLetters ?? []); setHistory(json.history ?? []); setFinanceFailures(json.financeFailures ?? []);
    } catch (loadError) {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card">
      <div className="border-b border-border p-5"><h2 className="font-black">Finance delivery failures</h2><p className="mt-1 text-xs text-muted-foreground">Failed finance sends are shown separately from accounting-state maintenance.</p></div>
      {financeFailures.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">No recent finance delivery failures.</p> : <div className="divide-y divide-border">{financeFailures.map((row) => <div key={row.id} className="p-4"><p className="text-sm font-bold">{row.stream} | {row.action}{row.channel ? ` | ${row.channel}` : ''}</p><p className="mt-1 text-xs text-rose-500">{row.error || 'Delivery failed without a provider message.'}</p><p className="mt-1 text-[11px] text-muted-foreground">{new Date(row.created_at).toLocaleString()}{row.entity_id ? ` | ${row.entity_id}` : ''}</p></div>)}</div>}
    </section>


    <section className="overflow-hidden rounded-2xl border border-border bg-card">
      <div className="border-b border-border p-5"><h2 className="font-black">Recent execution history</h2></div>
      <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-muted/40 text-muted-foreground"><tr><th className="p-3">Job</th><th className="p-3">Result</th><th className="p-3">Status</th><th className="p-3">Duration</th><th className="p-3">Time</th></tr></thead><tbody>{history.slice(0, 30).map((row) => <tr key={row.id} className="border-t border-border"><td className="p-3 font-bold">{row.job_name}</td><td className={`p-3 font-black ${row.success ? 'text-emerald-500' : 'text-rose-500'}`}>{row.success ? 'Success' : 'Failed'}</td><td className="p-3">{row.status_code ?? '-'}</td><td className="p-3">{row.duration_ms} ms</td><td className="p-3">{new Date(row.created_at).toLocaleString()}</td></tr>)}</tbody></table></div>
    </section>
  </div>;
}
