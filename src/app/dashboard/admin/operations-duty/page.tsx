...
export default function OperationsDutyPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-8">
      ...
      {board ? <>
        ...
        {board.warnings.map((warning) => <p key={warning} className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">{warning}</p>)}

        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <div className="border-b border-border p-5"><h2 className="font-black">Ranked duty capacity</h2><p className="mt-1 text-xs text-muted-foreground">The top person receives the next routine case. Complaints remain restricted to the effective admin.</p></div>
          <div className="divide-y divide-border">
            {board.ranked.map((row, index) => (
              <div key={row.id} className="flex flex-wrap items-center justify-between gap-4 p-5">
                <div className="min-w-0">
                  <p className="font-black">{index + 1}. {row.fullName} <span className="ml-2 text-xs font-bold uppercase text-muted-foreground">{row.role}</span></p>
                  <p className="mt-1 text-xs text-muted-foreground">{row.activeCases}/{row.maxActiveCases} active cases{row.teachesWithinMinutes != null ? ` ? teaches in ${row.teachesWithinMinutes} min` : ''}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{row.reasons.join(' ? ') || 'available by default'}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button disabled={saving === row.id} onClick={() => void updateAvailability(row)} className="rounded-lg bg-muted px-3 py-2 text-xs font-black">{row.isAvailable ? 'Mark away' : 'Mark available'}</button>
                  <button disabled={saving === row.id} onClick={() => void startDuty(row, false)} className="rounded-lg border border-primary px-3 py-2 text-xs font-black text-primary">Backup 8h</button>
                  <button disabled={saving === row.id} onClick={() => void startDuty(row, true)} className="rounded-lg bg-primary px-3 py-2 text-xs font-black text-white">Primary 8h</button>
                </div>
              </div>
            ))}
          </div>
...
function Metric({ label, value }: { label: string; value: string | number }) {
  return <div className="rounded-2xl border border-border bg-card p-5"><p className="text-xs font-bold uppercase text-muted-foreground">{label}</p><p className="mt-2 truncate text-2xl font-black">{value}</p></div>;
}
