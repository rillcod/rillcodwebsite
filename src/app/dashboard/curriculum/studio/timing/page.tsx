...
export default function CurriculumTimingPage() {
  ...
  async function load() {
    ...
    setAssignments(payload.data.assignments ?? []); setClasses(payload.data.classes ?? []); setSchedules(payload.data.schedules ?? []);
    setAssignmentId((current) => current || payload.data.assignments?.[0]?.id || '');
  }
  useEffect(() => { load().catch((reason) => setError(reason instanceof Error ? reason.message : 'Could not load school timing.')); }, []);
  ...
  async function save() {
    ...
    try {
      ...
      setMessage(payload.data.message); await load();
  ...
  return <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
    <header className="mb-7 rounded-3xl border border-cyan-500/20 bg-cyan-500/5 p-6"><p className="text-xs font-black uppercase tracking-widest text-cyan-600">Curriculum Studio</p><h1 className="mt-2 text-3xl font-black">School timing</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Tell the system when teaching actually begins. The official learning sequence stays intact, while each school or class starts from its real calendar position.</p></header>
    {message && <p className="mb-5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm font-bold text-emerald-700 dark:text-emerald-300">{message}</p>}{error && <p className="mb-5 rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm font-bold text-destructive">{error}</p>}
