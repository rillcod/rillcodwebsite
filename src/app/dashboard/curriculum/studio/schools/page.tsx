...
export default function SchoolDirectionPage() {
  ...
  const [form, setForm] = useState({ curriculum_id: '', academic_session: '2026/2027', audience_label: '', grade_key: '', effective_term_number: 1, change_summary: '', source_name: 'Rillcod Academic Office', framework: 'Rillcod Coding and Robotics Academic Standard', assign_eligible_schools: true });
  ...
  return (
    <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <Link href="/dashboard/curriculum/studio" className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground"><ArrowLeftIcon className="h-4 w-4" /> Curriculum Studio</Link>
      <header className="mb-7 rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 to-transparent p-6">
        <p className="text-xs font-black uppercase tracking-widest text-primary">Curriculum Studio</p><h1 className="mt-2 text-3xl font-black">Official direction and schools</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Protect one clear academic direction, then assign it safely. Each school can begin in a different term or week; existing class plans never change silently.</p>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1.15fr]">
        <section className="rounded-3xl border border-border bg-card p-5 sm:p-6">
          <div className="mb-5 flex items-start gap-3"><ShieldCheckIcon className="h-7 w-7 text-primary" /><div><h2 className="text-lg font-black">Protect the official academic direction</h2><p className="text-sm text-muted-foreground">The quality engine checks the draft again before publication.</p></div></div>
          <div className="space-y-4">
            ...
            <label className="flex gap-3 rounded-xl border border-border p-3 text-sm"><input type="checkbox" checked={form.assign_eligible_schools} onChange={(e) => setForm({ ...form, assign_eligible_schools: e.target.checked })} /><span><strong>Assign to eligible schools after protection</strong><span className="block text-muted-foreground">Only future class plans use it. Current teaching remains stable.</span></span></label>
            <button onClick={publish} disabled={busy || !form.curriculum_id} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 font-black text-primary-foreground disabled:opacity-50">{busy ? <ArrowPathIcon className="h-5 w-5 animate-spin" /> : <CheckCircleIcon className="h-5 w-5" />} Protect official direction</button>
          </div>
        </section>

        <section className="rounded-3xl border border-border bg-card p-5 sm:p-6">
          <div className="mb-5 flex items-start gap-3"><BuildingOfficeIcon className="h-7 w-7 text-primary" /><div><h2 className="text-lg font-black">Official directions</h2><p className="text-sm text-muted-foreground">Review school impact before assigning anything.</p></div></div>
      ...
      {preview && <section className="mt-6 rounded-3xl border border-primary/25 bg-primary/5 p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-lg font-black">School impact before assignment</h2><p className="mt-1 text-sm text-muted-foreground">{preview.summary.eligible} ready · {preview.summary.skipped} paused/not selected · {preview.summary.conflict} need attention · {preview.summary.protected_active_plans} current plans protected</p></div><button onClick={() => schoolImpact(preview.release.id, true)} disabled={busy || preview.summary.eligible === 0} className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-black text-primary-foreground disabled:opacity-50"><BuildingOfficeIcon className="h-5 w-5" /> Assign {preview.summary.eligible} schools</button></div><div className="mt-5 grid gap-3 md:grid-cols-2">{preview.schools.map((school) => <div key={school.school_id} className="rounded-xl border border-border bg-card p-3"><div className="flex items-center justify-between gap-3"><p className="font-bold">{school.school_name}</p><span className="rounded-full bg-muted px-2 py-1 text-xs font-bold">{school.rollout_status === 'eligible' ? 'Ready' : school.rollout_status === 'skipped' ? 'Paused' : 'Needs attention'}</span></div><p className="mt-2 text-xs text-muted-foreground">{school.active_plan_count > 0 ? `${school.active_plan_count} current class plan(s) will remain unchanged.` : 'No current class plan will be affected.'}</p></div>)}</div><div className="mt-5 flex items-center gap-2 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3 text-xs text-muted-foreground"><CalendarDaysIcon className="h-5 w-5 text-cyan-500" /> After assignment, set each school’s real entry point—for example Third Term, Week 3—without editing the curriculum.</div></section>}
    </main>
  );
}
