...
function CodingBlocksChallenge({ 
    ...
    return (
        <div className="space-y-4">
            <div className="p-4 bg-card shadow-sm border border-border rounded-xl flex flex-wrap items-center gap-x-2 gap-y-3 leading-loose">
                    ...
                    <div className="relative z-10 flex items-start justify-between gap-4 flex-wrap">
                            ...
                            {canGrade && (
                                <>
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 px-3 py-1.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] text-xs font-bold rounded-xl transition-colors border border-[#25D366]/30"
                        ...
                        {isStaff && (
                            <div className="flex gap-4 text-center flex-shrink-0">
                                <div>
                                    <p className="text-2xl font-black text-primary">{submitted}</p>
                    ...
                    <div className="space-y-4">
                        {Array.isArray(assignment.metadata.deliverables) && assignment.metadata.deliverables.length > 0 && (
                            <div className="bg-primary/5 border border-primary/20 rounded-xl p-6">
                                <h2 className="font-bold text-primary mb-3 flex items-center gap-2 text-sm uppercase tracking-widest">
                            ...
                            <form onSubmit={handleSubmit} className={`space-y-6 ${MOBILE_STICKY_ACTIONS_BOTTOM} sm:pb-0`}>
                                ...
                                {assignment.questions && Array.isArray(assignment.questions) && assignment.questions.length > 0 && (
                                    <div className="space-y-6">
                                        {assignment.questions.map((q: any, i: number) => (
                                            <div key={i} className="bg-card/90 backdrop-blur-2xl border border-border/80 rounded-3xl p-4 sm:p-6 shadow-xl space-y-3">
                                                ...
                                                {(q.question_type === 'essay' || q.question_type === 'fill_blank') && (
                                                    <input type="text" value={answers[i] || ''} onChange={e => setAnswers({ ...answers, [i]: e.target.value })} placeholder="Type your answer here…" className="w-full px-4 py-2.5 bg-card shadow-sm border border-border rounded-xl text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:border-amber-500 transition-colors" />
                                                )}
