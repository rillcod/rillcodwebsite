...
export default function MarkerIntegrityPage({
  ...
  const totalConflicts =
    (s?.assignment_duplicate_markers ?? 0) +
    (s?.deck_duplicate_markers ?? 0) +
    (s?.shared_markers_between_assignments_and_decks ?? 0);
  const isHealthy = totalConflicts === 0;

  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-10 mobile-page-root">
      {/* Guardian Header */}
      <div
        className={
          embedded
            ? "relative overflow-hidden bg-card border border-border rounded-xl p-4"
            : "relative overflow-hidden bg-card border border-border rounded-2xl p-8 sm:p-14 shadow-2xl"
        }
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-10 relative z-10">
          <div className={embedded ? "hidden" : "space-y-3"}>
            <h1 className="text-3xl font-black tracking-tight text-card-foreground leading-tight">
              System Health
            </h1>
            <p className="text-lg text-muted-foreground max-w-xl leading-relaxed italic">
              Checks that assignments and practice resources have safe, unique
              links so learner records never cross.
            </p>
          </div>
          <button
            type="button"
      ...
      <div
        className={`rounded-2xl border p-1.5 transition-all shadow-2xl ${
          isHealthy
      ...
      >
        <div className="bg-card rounded-[calc(2.5rem-1px)] p-10 flex flex-col sm:flex-row items-center gap-10">
          ...
          >
            {isHealthy ? (
              <div className="w-10 h-10 border-4 border-white rounded-full flex items-center justify-center font-black">
                ✓
              </div>
            ) : (
        ...
        {[
          {
            ...
            sub: `${s?.assignments_with_marker ?? 0} linked`,
          },
          {
            label: "Practice resources",
            value: s?.decks_total ?? 0,
            sub: `${s?.decks_with_marker ?? 0} linked`,
          },
          {
          ...
          },
          {
            ...
            value: s?.shared_markers_between_assignments_and_decks ?? 0,
            sub: "Shared references",
          },
        ].map((m, i) => (
          <div
            key={i}
            className="bg-card border border-border rounded-2xl p-8 space-y-3 shadow-xl hover:border-primary/30 transition-all group"
          >
        ...
        {[
          {
            ...
            desc: "Repeated assignment references.",
            data: data?.assignmentDuplicates,
            color: "amber",
          },
          {
            title: "Practice-resource links",
            desc: "Repeated practice-resource references.",
            data: data?.deckDuplicates,
            color: "amber",
          },
          {
            title: "Shared links",
            desc: "References used by more than one content type.",
            data: data?.sharedMarkers,
            color: "rose",
          },
        ].map((col, i) => (
          <div
            key={i}
            className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col shadow-2xl hover:border-primary/20 transition-all"
          >
            ...
            <div className="p-10 space-y-4 flex-1 overflow-auto max-h-[400px] bg-card/30 backdrop-blur-sm">
              {col.data?.length ? (
                col.data.map((row: any, idx: number) => (
                  <div
                    key={idx}
                    className="p-5 bg-background border border-border rounded-[1.5rem] space-y-3 shadow-sm hover:border-primary/20 transition-all group"
                  >
                    <p
                    ...
                    <div className="flex items-center justify-between border-t border-border pt-3">
                      <span className="text-xs font-black uppercase tracking-widest text-muted-foreground opacity-50">
                        Items affected
                      </span>
                      <span className="text-xs font-black text-foreground">
                        {row.count || row.assignments + row.decks}
                      </span>
                    </div>
                  </div>
                ))
                ...
                <div className="py-20 text-center">
                  <p className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em] opacity-30 italic">
                    No issues found
                  </p>
                </div>
              )}
