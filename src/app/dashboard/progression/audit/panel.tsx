...
function AuditTrailContent({ embedded = false }: { embedded?: boolean }) {
  ...
  const [lessonPlanId, setLessonPlanId] = useState(
    searchParams.get("lesson_plan_id") ?? ""
  );
  const [limit, setLimit] = useState(
    Math.min(200, Math.max(25, Number(searchParams.get("limit") ?? 50)))
  );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-8 space-y-12 mobile-page-root">
      ...
      >
        ...
        >
          <ArrowLeftIcon className="w-4 h-4" /> Back to Academic rules
        </Link>
      ...
      <div className="space-y-10 relative before:absolute before:left-[1.75rem] sm:before:left-[2.25rem] before:top-4 before:bottom-4 before:w-px before:bg-border/60">
        {rows.map((row) => {
          ...
          if (row.term_number)
            locationParts.push(
              `${TERM_LABELS[row.term_number] ?? row.term_number} Term`
            );
              ...
              <div className="absolute left-0 sm:left-4 top-2 w-12 h-12 sm:w-16 sm:h-16 rounded-[1.5rem] bg-card border border-border flex items-center justify-center z-10 group-hover:border-primary/50 transition-all duration-500 shadow-2xl">
                <div
                  className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full ${
                    meta.label === "Manual Unlock"
                      ? "bg-amber-400 animate-pulse"
                      : meta.label === "Locked Edit"
                      ? "bg-rose-400"
                      : "bg-primary shadow-[0_0_15px_rgba(124,58,237,0.5)]"
                  }`}
                />
              </div>
              ...
              <div className="bg-card border border-border rounded-[3rem] p-10 sm:p-14 space-y-10 hover:border-primary/20 transition-all duration-700 shadow-[0_30px_100px_rgba(0,0,0,0.05)] hover:shadow-[0_50px_150px_rgba(0,0,0,0.1)]">
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8">
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 flex-wrap">
                      <span
                        className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border shadow-sm ${meta.color}`}
                      >
                        {meta.label}
                      </span>
                    ...
                    <p className="text-2xl sm:text-3xl font-black text-foreground tracking-tight group-hover:text-primary transition-colors duration-500 leading-tight">
                      {meta.desc}
                    </p>
                ...
                {row.reason && (
                  <div className="p-8 bg-muted/10 border border-border/50 rounded-[2rem] shadow-inner relative overflow-hidden group/reason">
                    ...
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-4 opacity-50">
                      Reason
                    </p>
                    <p className="text-lg text-foreground leading-relaxed italic opacity-90">
                      "{row.reason}"
                    </p>
                  </div>
                )}
...
export default function ProgressionAuditPage({
  ...
  return (
    ...
    >
      <AuditTrailContent embedded={embedded} />
    </Suspense>
  );
}
