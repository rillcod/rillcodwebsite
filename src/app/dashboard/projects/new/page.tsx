...
const GRADING_MODES = [
    ...
    },
    {
        ...
        color: 'border-primary/40 bg-primary/10 text-violet-700 dark:text-violet-300',
    },
];
        ...
        <div className="min-h-screen bg-background">
            ...
            <div className="bg-[#0a0a12] border-b border-white/[0.06] px-6 md:px-10 py-5 sticky top-0 z-20">
                <div className="flex items-center justify-between max-w-6xl mx-auto">
                    <div className="flex items-center gap-4">
                        <Link href="/dashboard/projects" className="flex items-center gap-2 text-muted-foreground hover:text-white text-xs font-bold uppercase tracking-widest transition-colors">
                            <ArrowLeftIcon className="w-3.5 h-3.5" /> Projects
                        </Link>
                        <span className="text-muted-foreground">/</span>
                        <span className="text-muted-foreground text-xs font-bold uppercase tracking-widest">New Activity</span>
                    </div>
                                ...
                                <div key={s.n} className="flex items-center">
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-3 py-2 transition-all text-[10px] font-black uppercase tracking-widest border-b-2 ${
                                            ...
                                            done   ? 'border-emerald-500/60 text-emerald-600 dark:text-emerald-400 cursor-pointer hover:text-emerald-700 dark:hover:text-emerald-300' :
                                    ...
                                    {i < STEPS.length - 1 && <span className="text-muted-foreground px-1">›</span>}
                                </div>
                            );
                        })}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest md:hidden">Step {step}/4</span>
                        <button onClick={() => { setAiOpen(true); setAiError(''); }}
                            className="flex items-center gap-2 px-3 py-2 bg-primary/20 border border-primary/40 text-violet-700 dark:text-violet-300 text-[10px] font-black uppercase tracking-widest hover:bg-primary/30 transition-all">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/35 backdrop-blur-sm">
                    <div className="w-full max-w-lg bg-card border border-border shadow-2xl">
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                ...
                                {['Python calculator app', 'HTML portfolio page', 'Arduino LED project', 'AI chatbot with Python', 'Scratch animation story'].map(ex => (
                                    <button key={ex} onClick={() => setAiPrompt(ex)}
                                        className="text-[9px] text-primary/70 border border-primary/20 px-2 py-1 hover:border-primary/50 hover:text-violet-700 dark:hover:text-violet-300 transition-all">
                                        {ex}
                                    </button>
                                ))}
                            </div>
                            ...
                            {aiError && (
                                <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400 text-xs bg-rose-500/10 border border-rose-500/20 px-3 py-2">
                            ...
                            <p className="text-[9px] text-muted-foreground text-center">AI will fill in title, description, instructions, category, difficulty, and tags. You can edit everything after.</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {error && (
                <div className="flex items-center gap-3 bg-rose-500/10 border-b border-rose-500/20 px-6 md:px-10 py-3">
                    ...
                    <button onClick={() => setError('')} className="ml-auto text-rose-600/50 dark:text-rose-400/50 hover:text-rose-600 dark:hover:text-rose-400">✕</button>
                </div>
            )}
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {step === 1 && (
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-lg font-black text-foreground uppercase tracking-tight italic mb-0.5">Activity Details</h2>
                                    <p className="text-xs text-muted-foreground">Set up the core information about this project activity</p>
                                </div>

                                <div className={CARD}>
                                    <div className="space-y-4">
                                        <div>
                                            <label className={LABEL}>Activity Title <span className="text-rose-600 dark:text-rose-400">*</span></label>
                                            <input
                                                value={title}
                                            ...
                                            {title.length > 0 && title.trim().length < 3 && (
                                                <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-1">Title must be at least 3 characters</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1.5 mb-2 min-h-[28px]">
                                                {tagList.map(t => (
                                                    <span key={t} className="flex items-center gap-1 px-2 py-0.5 bg-primary/20 border border-primary/40 text-violet-700 dark:text-violet-300 text-[10px] font-black">
                                                        {t}
                                                        <button type="button" onClick={() => setTagList(prev => prev.filter(x => x !== t))} className="text-primary/50 hover:text-rose-600 dark:hover:text-rose-400 transition-colors leading-none ml-0.5">×</button>
                                                    </span>
                                                ))}
                                                {tagList.length === 0 && <span className="text-[10px] text-muted-foreground italic">No tags yet — pick below or type a custom one</span>}
                                            </div>
                                                ...
                                                {COMMON_TAGS.map(t => {
                                                    ...
                                                    return (
                                                        <button key={t} type="button"
                                                            ...
                                                            className={`px-2 py-0.5 text-[9px] font-bold border transition-all ${active ? 'bg-primary/20 border-primary/40 text-violet-700 dark:text-violet-300' : 'bg-white/[0.02] border-white/[0.06] text-muted-foreground hover:border-white/20 hover:text-white/60'}`}>
                                                            {active ? '✓ ' : ''}{t}
                                                        </button>
                                                    );
                                                })}
                                            ...
                                            <div className="flex gap-2">
                                                <input value={customTagInput} onChange={e => setCustomTagInput(e.target.value)}
                                                    ...
                                                    className="flex-1 px-3 py-1.5 bg-white/5 border border-white/10 text-xs text-foreground placeholder-white/20 focus:outline-none focus:border-primary transition-colors"
                                                />
                                                <button type="button"
                                                    onClick={() => {
                                                        const t = customTagInput.trim().toLowerCase().replace(/\s+/g, '-');
                                                    ...
                                                    className="px-3 py-1.5 bg-white/5 border border-white/10 text-muted-foreground text-xs hover:border-white/20 transition-colors">
                                                    + Add
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        {CATEGORIES.map(cat => {
                                            ...
                                            return (
                                                <button key={cat.key} type="button" onClick={() => setCategory(cat.key)}
                                                    ...
                                                    <span className={`text-[10px] font-black uppercase tracking-widest ${active ? 'text-white' : 'text-muted-foreground'}`}>{cat.label}</span>
                                        ...
                                        {DIFFICULTIES.map(d => {
                                            ...
                                            return (
                                                <button key={d.key} type="button" onClick={() => setDifficulty(d.key)}
                                                    className={`flex flex-col gap-2 px-4 py-4 border text-left transition-all relative ${active ? d.color : 'border-white/[0.06] bg-white/[0.02] text-muted-foreground hover:border-white/20 hover:bg-white/[0.03]'}`}>
                                                    {/* Stars */}
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className={`text-[10px] mt-1 leading-tight ${active ? 'opacity-70' : 'text-muted-foreground'}`}>{d.sub}</p>
                                                    </div>
                                                    <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 self-start border ${active ? 'border-current opacity-50' : 'border-white/10 text-muted-foreground'}`}>
                                                        {d.badge}
                                                    </span>
                                                    {active && (
                        ...
                        {step === 2 && (
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-lg font-black text-foreground uppercase tracking-tight italic mb-0.5">Submission & Grading</h2>
                                    <p className="text-xs text-muted-foreground">Choose how students submit work and how it will be graded</p>
                                </div>

                                {/* Submission types */}
                                <div className={CARD}>
                                    <label className={LABEL}>Allowed Submission Types <span className="text-rose-600 dark:text-rose-400">*</span></label>
                                    <p className="text-[10px] text-muted-foreground mb-3">Select at least one. Students will see only these options.</p>
                                        ...
                                        {SUBMISSION_TYPES.map(t => {
                                            ...
                                            return (
                                                <button key={t.key} type="button"
                                                    ...
                                                    <div className="flex-1">
                                                        <p className={`text-xs font-black uppercase tracking-widest ${active ? 'text-white' : 'text-muted-foreground'}`}>{t.label}</p>
                                    ...
                                    {selectedTypes.length === 0 && (
                                        <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-2">Select at least one submission type</p>
                                    )}
                                </div>
                                        ...
                                        {GRADING_MODES.map(gm => {
                                            ...
                                            return (
                                                <button key={gm.key} type="button" onClick={() => setGradingMode(gm.key as any)}
                                                    className={`w-full flex items-start gap-4 px-4 py-4 border text-left transition-all ${active ? gm.color : 'border-white/[0.06] bg-white/[0.02] hover:border-white/20 text-muted-foreground'}`}>
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-[10px] text-muted-foreground mt-1">{gm.desc}</p>
                                                        {active && gm.key === 'auto' && gm.breakdown && (
                                                            <div className="mt-2 grid grid-cols-2 gap-1">
                                                                {gm.breakdown.map(b => (
                                                                    <span key={b} className="text-[9px] text-emerald-600/70 dark:text-emerald-400/70">• {b}</span>
                                                                ))}
                                                            </div>
                                                        )}
                                                    </div>
                                ...
                                {gradingMode !== 'rubric' ? (
                                    <div className={CARD}>
                                        ...
                                        <p className="text-[10px] text-muted-foreground mt-1.5">This is the maximum score a student can receive for this activity</p>
                                    </div>
                                ) : (
                                    <div className={CARD}>
                                        <div className="flex items-center justify-between mb-4">
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted-foreground mt-0.5">Total: <span className="text-primary font-black">{rubricTotal} pts</span></p>
                                            </div>
                                                ...
                                                <div key={c.id} className="bg-primary/5 border border-primary/20 p-4 space-y-2">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <input value={c.name} onChange={e => updateCriterion(c.id, 'name', e.target.value)}
                                                            ...
                                                            className="flex-1 bg-transparent border-b border-white/10 text-sm font-black text-foreground focus:outline-none focus:border-primary pb-0.5 transition-colors" />
                                                        <div className="flex items-center gap-1 flex-shrink-0">
                                                            <input type="number" min="1" max="200" value={c.maxPts} onChange={e => updateCriterion(c.id, 'maxPts', parseInt(e.target.value) || 0)}
                                                                className="w-14 text-center bg-primary/10 border border-primary/30 text-violet-700 dark:text-violet-300 text-sm font-black focus:outline-none py-0.5" />
                                                            <span className="text-[10px] text-muted-foreground font-bold">pts</span>
                                                        </div>
                                                        {rubric.length > 1 && (
                                                            <button type="button" onClick={() => removeCriterion(c.id)} className="text-rose-600/40 dark:text-rose-400/40 hover:text-rose-600 dark:hover:text-rose-400 transition-colors flex-shrink-0">
                                                    ...
                                                    <input value={c.desc} onChange={e => updateCriterion(c.id, 'desc', e.target.value)}
                                                        ...
                                                        className="w-full bg-transparent text-[11px] text-muted-foreground focus:outline-none focus:text-white/60 transition-colors pl-7" />
                                                </div>
                                            ))}
                                        </div>
                                        {rubricTotal !== 100 && (
                        ...
                        {step === 3 && (
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-lg font-black text-foreground uppercase tracking-tight italic mb-0.5">Visibility & Assignment</h2>
                                    <p className="text-xs text-muted-foreground">Control who sees this activity and how they work on it</p>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {([
                                            ...
                                            <button key={opt.key} type="button" onClick={() => setVisibilityType(opt.key)}
                                                ...
                                                <div>
                                                    <p className={`text-xs font-black uppercase tracking-widest ${visibilityType === opt.key ? 'text-violet-700 dark:text-violet-300' : 'text-muted-foreground'}`}>{opt.label}</p>
                                    ...
                                    {visibilityType === 'class' && (
                                        <div className="mt-4">
                                            ...
                                            {classesLoading ? (
                                                ...
                                                <p className="text-xs text-muted-foreground italic py-2">No classes found for this school</p>
                                            ) : (
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mt-1">
                                        {([
                                            ...
                                            <button key={opt.key} type="button"
                                                ...
                                                <div>
                                                    <p className={`text-[10px] font-black uppercase tracking-widest ${workMode === opt.key ? 'text-indigo-700 dark:text-indigo-300' : 'text-muted-foreground'}`}>{opt.label}</p>
                                                    <p className="text-[9px] text-muted-foreground mt-1 leading-relaxed">{opt.sub}</p>
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                                            ...
                                            {filteredStudents.map(s => {
                                                ...
                                                return (
                                                    <button key={s.id} type="button" onClick={() => toggleTargetStudent(s.id)}
                                                        className={`flex items-center gap-1.5 px-2 py-1.5 border text-[10px] font-bold text-left transition-all ${sel ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-700 dark:text-indigo-300' : 'bg-white/[0.02] border-white/[0.06] text-muted-foreground hover:border-indigo-500/30 hover:text-white'}`}>
                                                        {sel ? <CheckIcon className="w-2.5 h-2.5 flex-shrink-0 text-indigo-600 dark:text-indigo-400" /> : <div className="w-2.5 h-2.5 rounded-full border border-white/20 flex-shrink-0" />}
                                ...
                                {workMode === 'group' && (
                                    <div className={CARD}>
                                                ...
                                                <div key={group.id} className="border border-white/[0.08] bg-black/20">
                                                    ...
                                                    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-white/[0.06] bg-indigo-500/5">
                                                        <div className="w-6 h-6 bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center flex-shrink-0">
                                                            <span className="text-[9px] font-black text-indigo-600 dark:text-indigo-400">{gi + 1}</span>
                                                        </div>
                                                        <input value={group.name}
                                                            onChange={e => setGroups(gs => gs.map(g => g.id === group.id ? { ...g, name: e.target.value } : g))}
                                                            className="flex-1 bg-transparent text-sm font-black text-foreground focus:outline-none" placeholder="Team name..." />
                                                        <span className="text-[10px] text-muted-foreground flex-shrink-0">
                                                            {group.studentIds.length} member{group.studentIds.length !== 1 ? 's' : ''}
                                                        </span>
                                                        {groups.length > 1 && (
                                                            <button type="button" onClick={() => removeGroup(group.id)} className="text-rose-600/40 dark:text-rose-400/40 hover:text-rose-600 dark:hover:text-rose-400 transition-colors flex-shrink-0">
                                                                <TrashIcon className="w-3.5 h-3.5" />
                                                            </button>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="p-3 max-h-52 overflow-y-auto">
                                                        {filteredStudents.length === 0 && (
                                                            <p className="text-muted-foreground text-xs italic">No students found</p>
                                                        )}
                                                                ...
                                                                return (
                                                                    <button key={s.id} type="button" disabled={inOther}
                                                                        ...
                                                                        className={`flex items-center gap-1.5 px-2 py-1.5 border text-[10px] font-bold text-left transition-all ${
                                                                            inThis  ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-700 dark:text-indigo-300' :
                                                                            inOther ? 'opacity-25 border-white/5 text-muted-foreground cursor-not-allowed' :
                                                                                      'bg-white/[0.02] border-white/[0.06] text-muted-foreground hover:border-indigo-500/30 hover:text-white'
                                                                        }`}>
                                                                        {inThis
                                                                            ? <CheckIcon className="w-2.5 h-2.5 flex-shrink-0 text-indigo-600 dark:text-indigo-400" />
                                        ...
                                        <button type="button" onClick={addGroup}
                                            className="mt-3 w-full flex items-center justify-center gap-2 py-2.5 border border-dashed border-white/20 text-muted-foreground hover:text-white hover:border-white/40 text-xs font-bold uppercase tracking-widest transition-all">
                                ...
                                <div className="bg-[#0d0d18] border border-primary/20 overflow-hidden">
                                    ...
                                    <div className="p-6 space-y-5">
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                <h3 className="text-xl font-black text-foreground italic">{title || 'Untitled Activity'}</h3>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                            {[
                                                ...
                                                <div key={s.label} className="bg-white/[0.03] border border-white/[0.06] px-3 py-2.5">
                                                    ...
                                                    <p className="text-xs font-black text-foreground mt-0.5">{s.value}</p>
                                                </div>
                                            ))}
                                        </div>
                                                ...
                                                <div className="space-y-1.5">
                                                    {rubric.map(c => (
                                                        <div key={c.id} className="flex items-center justify-between bg-primary/5 border border-primary/20 px-3 py-2">
                                                            <div>
                                                                <p className="text-xs font-black text-foreground">{c.name || 'Unnamed'}</p>
                                                                {c.desc && <p className="text-[10px] text-muted-foreground">{c.desc}</p>}
                                                            </div>
                                                    ...
                                                    <div className="flex items-center justify-between px-3 py-1.5">
                                                        ...
                                                        <span className="text-sm font-black text-foreground">{rubricTotal} pts</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {instructions && (
                                            <div>
                                                ...
                                                <div className="bg-white/[0.02] border border-white/[0.06] p-4 max-h-40 overflow-y-auto">
                                                    <p className="text-[11px] text-muted-foreground leading-relaxed whitespace-pre-line">{instructions}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div className="flex items-center justify-between pt-2">
                                {step > 1 ? (
                                    <button type="button" onClick={() => setStep(s => s - 1)}
                                        className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 text-muted-foreground hover:text-white text-xs font-black uppercase tracking-widest transition-all">
                                        <ChevronLeftIcon className="w-3.5 h-3.5" /> Back
                                    </button>
                                ) : (
                                    <Link href="/dashboard/projects" className="text-muted-foreground hover:text-white text-xs font-bold uppercase tracking-widest transition-colors">Cancel</Link>
                                )}
                                <button type="button" onClick={() => { if (canProceed()) { setError(''); setStep(s => s + 1); } else { setError(step === 1 ? 'Enter a title (min 3 chars) to continue' : 'Select at least one submission type'); } }}
                                    className={`flex items-center gap-2 px-6 py-2.5 font-black uppercase tracking-widest text-sm transition-all ${canProceed() ? 'bg-primary hover:bg-primary text-white' : 'bg-white/5 border border-white/10 text-muted-foreground cursor-not-allowed'}`}>
                    ...
                    <div className="hidden lg:block">
                        <div className="sticky top-24 space-y-4">
                            <div className="bg-[#0a0a12] border border-white/[0.06] p-5">
                                <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-3">Live Preview</p>
                                ...
                                <div className="bg-[#0d0d18] border border-white/[0.06] overflow-hidden">
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-start justify-between gap-2 mb-2">
                                            <p className="text-xs font-black text-foreground line-clamp-2 leading-tight">{title || 'Activity Title'}</p>
                                            {isGroupActivity && <span className="text-[8px] font-black text-cyan-600 dark:text-cyan-400 px-1.5 py-0.5 bg-cyan-500/10 flex-shrink-0">Group</span>}
                                        </div>
                                        {description && <p className="text-[10px] text-muted-foreground line-clamp-2 mb-2">{description}</p>}
                                        ...
                                        {tagList.length > 0 && (
                                            <div className="flex flex-wrap gap-1 mb-2">
                                                {tagList.slice(0, 4).map(t => (
                                                    <span key={t} className="text-[8px] text-muted-foreground border border-white/10 px-1 py-0.5">{t}</span>
                                                ))}
                                            </div>
                                        )}
                                ...
                                <div className="mt-4 space-y-2">
                                    {[
                                        ...
                                        <div key={item.label} className="flex items-center gap-2">
                                            <div className={`w-4 h-4 flex items-center justify-center flex-shrink-0 ${item.done ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground'}`}>
                                                {item.done ? <CheckCircleIcon className="w-4 h-4" /> : <div className="w-3 h-3 border border-current rounded-full" />}
                                            </div>
                                            <span className={`text-[10px] font-semibold ${item.done ? 'text-muted-foreground' : 'text-muted-foreground'}`}>{item.label}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-amber-500/5 border border-amber-500/20 p-4">
                                <p className="text-[9px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest mb-2">💡 Tips</p>
                                {step === 1 && <p className="text-[10px] text-muted-foreground">Write clear, specific instructions — students perform better when expectations are explicit.</p>}
                                {step === 2 && <p className="text-[10px] text-muted-foreground">Use Rubric-Based grading for complex projects where multiple dimensions matter.</p>}
                                {step === 3 && <p className="text-[10px] text-muted-foreground">Group work builds teamwork skills — assign 3–5 students per team for best results.</p>}
                                {step === 4 && <p className="text-[10px] text-muted-foreground">Save as Draft to review with colleagues before releasing to students.</p>}
                            </div>
                        </div>
                    </div>
                </div>
