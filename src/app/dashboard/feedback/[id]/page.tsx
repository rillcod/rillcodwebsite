...
export default function FeedbackDetailPage() {
  ...
  if (error && !feedback) return <div className="p-8"><p className="text-red-600">{error}</p><Link href="/dashboard/feedback" className="text-primary underline">Back to feedback</Link></div>;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/dashboard/feedback" className="text-sm font-semibold text-primary">Back to feedback</Link>
          <p className="mt-4 text-xs font-black uppercase tracking-widest text-muted-foreground">FB-{feedback.id.slice(0, 8)}</p>
          <h1 className="mt-1 text-3xl font-black text-foreground">{feedback.subject}</h1>
          <p className="mt-2 text-sm text-muted-foreground">Submitted by {feedback.user_name} on {new Date(feedback.created_at).toLocaleString()}</p>
        </div>
        <span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-wider ${statusClasses[feedback.status] || statusClasses.new}`}>
      ...
      <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="mb-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
          ...
          {canRespond && feedback.user_email ? <span className="rounded-full bg-muted px-3 py-1">{feedback.user_email}</span> : null}
        </div>
      ...
      {feedback.admin_response && !canRespond ? (
        <section className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-6">
          ...
          {feedback.responded_at ? <p className="mt-4 text-xs text-muted-foreground">Responded {new Date(feedback.responded_at).toLocaleString()}</p> : null}
        </section>
      ) : null}
      ...
      {canRespond ? (
        <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">The response is saved to the case, shown in-app, and emailed when an address is available.</p>
          <textarea
            value={response}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <select value={status} onChange={(event) => setStatus(event.target.value as typeof status)} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
              <option value="in_progress">In progress</option>
              <option value="resolved">Resolved</option>
              <option value="closed">Closed</option>
            </select>
            <button onClick={saveResponse} disabled={saving || !response.trim()} className="rounded-xl bg-primary px-5 py-3 text-sm font-black text-white disabled:opacity-50">
