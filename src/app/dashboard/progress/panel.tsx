...
export default function ProgressPage({
        ...
        if (isStaff) {
          ...
          if (role === "school" && p.school_id) {
            ...
            const { data: assignments } = await supabase
              ...
              .eq("teacher_id", p.id);
            const ids =
              assignments?.map((a: any) => a.school_id).filter(Boolean) || [];
            ...
            if (assignedSchoolIds.length > 0) {
              ...
              assignedSchoolNames =
                namesData?.map((s) => s.name).filter(Boolean) || [];
            }
          }

          // 3. Fetch portal users via API (bypasses RLS) for name/school enrichment
          const portalUsersJson = await fetch(
          ...
          const [rawSubsRes, enrRes] = await Promise.allSettled([
            rawSubsQ,
            enrQuery,
          ]);
          ...
          const subsData = filterByAssignmentSession(
            (rawSubsRes.status === "fulfilled"
              ? rawSubsRes.value.data ?? []
              : []
            ).map((s: any) => ({
              ...s,
              portal_users: umap[s.portal_user_id ?? s.user_id] ?? null,
            })),
            liveTermId
          );

          const enrData = (
            enrRes.status === "fulfilled" ? enrRes.value.data ?? [] : []
          ).map((e: any) => {
          ...
          if (role !== "admin") {
            if (
              ...
              filteredSubs = subsData.filter((s: any) => {
                ...
                if (!u) return false;
                return (
                  assignedSchoolIds.includes(u.school_id) ||
                  assignedSchoolNames.includes(u.school_name)
                );
              });
          ...
          const [subsRes, enrRes] = await Promise.allSettled([
            supabase
              .from("assignment_submissions")
              .select(
                `id, grade, status, submitted_at, graded_at, feedback,
          ...
          if (!cancelled) {
            setSubmissions(
              subsRes.status === "fulfilled"
                ? filterByAssignmentSession(
                    (subsRes.value.data ?? []) as any[],
                    liveTermId
                  )
                : []
            );
        ...
        try {
          if (profile?.role === "student") {
            const { data: reports } = await supabase
              ...
              .select(
                "report_term, report_period, report_date, overall_score, theory_score, practical_score, course_name"
              )
            ...
            let rQuery = supabase
              .from("student_progress_reports")
              .select(
                "report_term, report_period, report_date, overall_score, theory_score, practical_score, course_name, student_name, school_name"
              )
              .eq("is_published", true)
              .order("report_date", { ascending: true })
              .limit(200);
  ...
  const filteredEnrollments = enrollments.filter((e) => {
    const u = e.portal_users || {};
    if (selectedSchool !== "all" && u.school_id !== selectedSchool)
      return false;
  ...
  const graded = activeSubs.filter(
  ...
  const best = graded.length
    ? Math.round(
        Math.max(
          ...graded.map(
            (s) => (s.grade / (s.assignments?.max_points ?? 100)) * 100
          )
        )
      )
    : null;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-card shadow-sm border border-border rounded-xl h-32 animate-pulse"
              />
            ))}
          </div>
          {[1, 2].map((i) => (
            <div
              key={i}
              className="bg-card shadow-sm border border-border rounded-xl h-40 animate-pulse"
            />
          ))}
        </div>
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div
          ...
          } items-center justify-between print:hidden`}
        >
          <div>
            ...
            <div className="lg:col-span-3 space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div
                    key={s.label}
                    className="bg-card shadow-sm border border-border rounded-xl p-4 group hover:border-border transition-all"
                  >
                    <div
                      className={`w-8 h-8 ${s.bg} rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform`}
                    >
            ...
            <div className="divide-y divide-white/5">
              {activeEnr.slice(0, 100).map((e: any) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="p-5 flex items-center gap-4 hover:bg-card shadow-sm transition-colors"
                  >
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="p-5 border-b border-border flex items-center gap-2">
              ...
              <h3 className="font-bold text-foreground">
                {isStaff
                  ? "Score Trends (Published Reports)"
                  : "My Score Trends"}
              </h3>
              <span className="ml-auto text-xs text-muted-foreground">
                {scoreReports.length} report
                {scoreReports.length !== 1 ? "s" : ""}
              </span>
            </div>
            <div className="p-5">
              <ResponsiveContainer width="100%" height={220}>
                <LineChart
                  data={scoreReports.map((r) => ({
                    term:
                      [r.report_term, r.report_period]
                        .filter(Boolean)
                        .join(" · ") ||
          ...
          <div className="bg-card shadow-sm border border-border rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {activeSubs.slice(0, 100).map((s: any) => {
                ...
                const pct =
                  s.grade != null ? Math.round((s.grade / max) * 100) : null;
                return (
                  <div
                    key={s.id}
                    className="p-4 flex items-center gap-4 hover:bg-card shadow-sm transition-colors"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      {isStaff && s.portal_users && (
                        <p className="text-xs text-muted-foreground">
                          {s.portal_users.full_name}
                        </p>
                      )}
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {s.assignments?.courses?.title}
                        {s.assignments?.courses?.programs?.name
                          ? ` · ${s.assignments.courses.programs.name}`
                          : ""}
                      </p>
                    </div>
          ...
          <div className="text-center py-20 bg-card shadow-sm border border-border rounded-xl">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {isStaff
                ...
                : "Submit assignments to start tracking your progress."}
            </p>
          </div>
        )}
      </div>
