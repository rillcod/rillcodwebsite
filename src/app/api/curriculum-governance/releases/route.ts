...
export async function GET(req: NextRequest) {
  ...
  let query = db
    ...
    .select('id, course_id, source_curriculum_id, release_number, title, change_summary, content_hash, source_metadata, status, published_at, published_by, courses(title, programs(name))')
  ...
  if (actor.role === 'admin') {
    const result = await db
      ...
      .select('id, course_id, version, content, updated_at, courses(title, programs(name))')
      .is('school_id', null)
  ...
  const [{ count: pending }, { count: adoptions }] = await Promise.all([
    ...
    db.from('academic_curriculum_adoptions').select('id', { count: 'exact', head: true }).eq('status', 'active'),
  ]);
  return NextResponse.json({
    data: releases ?? [],
    curricula,
    summary: { pending_proposals: pending ?? 0, active_adoptions: adoptions ?? 0 },
  });
}
...
export async function POST(req: NextRequest) {
  ...
  if (actor.role !== 'admin') {
    return NextResponse.json({ error: 'Only the academic administrator can publish official curriculum releases.' }, { status: 403 });
  }
  ...
  if (!curriculumId || !changeSummary) {
    return NextResponse.json({ error: 'curriculum_id and change_summary are required.' }, { status: 400 });
  }
  ...
  if (curriculum.school_id) {
    return NextResponse.json({ error: 'Official releases must be published from the central platform curriculum, not a school copy.' }, { status: 409 });
  }
  const content = curriculum.content ?? {};
  if (!Array.isArray(content?.terms) || content.terms.length === 0) {
    return NextResponse.json({ error: 'The curriculum must contain at least one term before publication.' }, { status: 400 });
  }
  ...
  if (body.rollout === true) {
    rollout = await applyCurriculumRollout({
