...
import {
  calendarIndex,
  sourceWeekIndexForCalendar,
} from "@/lib/qa/rotatedSpineIndex";
...
export async function POST(req: NextRequest) {
  ...
  const bodyProgramStartTerm = [1, 2, 3].includes(
    Number(body.program_start_term)
  )
    ? Number(body.program_start_term)
    : 0; // 0 = auto-detect from program policy
  ...
  const weekOverrides: Record<string, string> =
    body.week_overrides &&
    typeof body.week_overrides === "object" &&
    !Array.isArray(body.week_overrides)
  ...
  const { data: curriculum, error: currErr } = await supabase
    ...
    .select(
      "id, course_id, school_id, content, version, courses(program_id, title)"
    )
    .eq("id", curriculumId)
    .single();
  ...
  const programId =
    (
      curriculum as unknown as {
        courses?: { program_id?: string | null } | null;
      }
    ).courses?.program_id ?? null;
  ...
  if (classRow) {
    ...
    const { data: off, error: rpcErr } = await supabase.rpc(
    ...
    if (rpcErr) {
      return NextResponse.json(
        { error: rpcErr.message, hint: "Run migration: class_qa_path_offset" },
        { status: 500 }
      );
    }
  ...
  if (programId) {
    const { data: programRows, error: trErr } = await supabase
      ...
      .select(
        "week_index, year_number, term_number, week_number, topic, subtopics, program_id"
      )
  ...
  if (!programStartTerm && programId) {
    const { data: prog } = await supabase
      ...
      .eq("id", programId)
      .maybeSingle();
    const saved = (prog?.progression_policy as Record<string, unknown> | null)
      ?.program_start_term;
  ...
  function buildTermWeeks(
    ...
    for (let w = 1; w <= 12; w++) {
      ...
      const topic =
        weekOverrides[overrideKey] ||
        row?.topic ||
        `Week ${w} (missing spine row ${src})`;
      out.push({
        week: w,
        type: "lesson",
        topic,
        subtopics: weekOverrides[overrideKey]
          ? []
          : toSubtopics(row?.subtopics),
        lesson_plan: null,
      });
    }
    return out;
  ...
  const content = asObject(
    (curriculum as { content?: unknown }).content
  ...
  const nextTerms = [1, 2, 3].map((nationalTermNo) => {
    ...
    const existing = baseTerms.find(
      (t) => Number(t?.term ?? 0) === nationalTermNo
    ) ?? { term: nationalTermNo };
    const existingWeeks = Array.isArray(existing.weeks) ? existing.weeks : [];
    return {
      ...existing,
      term: nationalTermNo,
      weeks: overwriteExisting
        ? incoming
  ...
  (nextContent as Record<string, unknown>).metadata = {
    ...
    qa_spine: {
      ...
      lane_index: chosenLane,
      lane_source:
        manualLane > 0 ? "body" : classId ? resolved.source : "default",
  ...
  return NextResponse.json({
    data: {
      ...
      qa_spine: {
        ...
        class_id: classId || null,
        path_offset: classRow ? pathOffset : 0,
        terms_weeks: nextTerms.reduce(
          (acc, t) => ({ ...acc, [`term_${t.term}`]: t.weeks.length }),
