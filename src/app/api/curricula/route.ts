...
function buildSchoolPrompt(
  ...
  const termLines = selectedTerms
    .map(
      (t) =>
        `  - Term ${t} — ${NG_TERM_LABEL[t] ?? `Term ${t}`}: ${
          themes[t] ?? "Progressive content"
        }`
    )
    .join("\n");
...
Target weeks per term: ${weeksPerTerm}. Use this as a GUIDE for pacing and assessment placement only — do not pad or cut topics artificially. Content determines length; a term may run ${
    weeksPerTerm - 1
  }–${weeksPerTerm + 1} lesson weeks if the subject matter demands it.
...
function buildBootcampPrompt(
  ...
  const sessionsMap: Record<
    string,
    { perWeek: number; label: string; mins: number }
  > = {
    fulltime: {
      perWeek: 5,
      label: "Full-time (5 days/week, ~6 hrs/day)",
      mins: 360,
    },
    parttime: {
      perWeek: 3,
      label: "Part-time (3 days/week, ~4 hrs/day)",
      mins: 240,
    },
    weekend: {
      perWeek: 2,
      label: "Weekend-only (Sat + Sun, ~8 hrs/day)",
      mins: 480,
    },
    evening: {
      perWeek: 3,
      label: "Evening (3 evenings/week, ~2 hrs/session)",
      mins: 120,
    },
  };
...
- Each term has ${
    s.perWeek
  } "week" entries (one per session that week; week: 1 … ${s.perWeek}).
...
- Each module has ${
    weeksPerModule * sessionsPerWeek
  } session entries ("weeks" in the JSON, numbered 1 onwards within the module).
...
Course: "${courseName}" | Audience: ${gradeLevel} | Subject Area: ${subjectArea}
Modules: ${modules} | Estimated time per module: ${hoursPerModule} hour${
    hoursPerModule > 1 ? "s" : ""
  }
...
function buildCurriculumPrompt(
  ...
  switch (format) {
    case "bootcamp":
      return buildBootcampPrompt(
        courseName,
        gradeLevel,
        subjectArea,
        opts.bootcampDurationWeeks ?? 4,
        opts.bootcampSchedule ?? "fulltime",
        notes
      );
    case "online":
      return buildOnlinePrompt(
        courseName,
        gradeLevel,
        subjectArea,
        opts.onlineDurationWeeks ?? 8,
        opts.onlineSessionsPerWeek ?? 2,
        notes
      );
    case "selfpaced":
      return buildSelfpacedPrompt(
        courseName,
        gradeLevel,
        subjectArea,
        opts.selfpacedModules ?? 6,
        opts.selfpacedHoursPerModule ?? 2,
        notes
      );
    default:
      return buildSchoolPrompt(
        courseName,
        gradeLevel,
        subjectArea,
        opts.selectedTerms ?? [1, 2, 3],
        opts.weeksPerTerm ?? 8,
        opts.programStartTerm ?? 1,
        notes,
        opts.previousTermsContext,
        opts.yearNumber ?? 1
      );
  }
}
...
function safeParseJSON(raw: string): any {
  ...
  if (brace !== -1 && lastBrace !== -1) {
    try {
      return JSON.parse(raw.slice(brace, lastBrace + 1));
    } catch {}
  }
  return null;
}
...
async function generateCurriculum(prompt: string): Promise<any> {
  ...
  if (process.env.GEMINI_API_KEY) {
    const SYSTEM_PROMPT =
      "You are an expert curriculum designer for Rillcod Technologies.";
    ...
    if (geminiResult?.text) {
      try {
        ...
        console.warn(
          "Direct Google Gemini curriculum parse failed, falling back to OpenRouter...",
          e
        );
      }
    }
...
export async function GET(req: NextRequest) {
  ...
  if (role === "student") {
    ...
    if (childIds.length > 0) {
      const { data: children } = await adminForLinks
        ...
        .select("school_id")
        .in("id", childIds);
      learnerSchoolIds = Array.from(
        new Set(
          (children ?? []).map((row: any) => row.school_id).filter(Boolean)
        )
      );
      const { data: progs } = await supabase
        .from("student_level_enrollments")
      ...
      const ids = Array.from(
        new Set(
          (progs ?? []).map((r) => r.course_id).filter(Boolean) as string[]
        )
      );
      if (ids.length > 0) learnerCourseIds = ids;
    }
  ...
  if (role === "student" || role === "parent") {
    if (learnerCourseIds && learnerCourseIds.length > 0) {
      ...
      const schoolFilters = [
        "school_id.is.null",
        ...learnerSchoolIds.map((sid) => `school_id.eq.${sid}`),
      ];
...
export async function POST(req: NextRequest) {
  ...
  const { data: profile } = await supabase
    ...
    .eq("id", user.id)
    .single();
  // The Academic Office owns official curriculum source authoring.
  // Teachers and schools consume assigned releases and propose changes.
  if (!profile || profile.role !== "admin") {
    return NextResponse.json(
      {
        error:
          "Only the Academic Office can create or update the official curriculum source. Teachers customise teaching plans or submit a change proposal.",
      },
      { status: 403 }
    );
  }
  ...
  const {
    ...
    bootcamp_schedule,
    // Online
    online_duration_weeks,
    online_sessions_per_week,
    // Self-paced
    selfpaced_modules,
    selfpaced_hours_per_module,
    // Format
    format: rawFormat,
  } = body;
  if (!course_name)
  ...
  const resolvedYear = [1, 2, 3].includes(Number(programme_year))
    ? Number(programme_year)
    : 1;
  const resolvedStartTerm = [1, 2, 3].includes(Number(program_start_term))
    ? Number(program_start_term)
    : 1;
  ...
  if (format === "school") {
    if (Array.isArray(selected_terms) && selected_terms.length > 0) {
      ...
      if (!termNums.length)
        return NextResponse.json(
          { error: "selected_terms must include at least one of 1, 2, 3" },
          { status: 400 }
        );
    } else {
      const tc = Number(term_count ?? 3);
      if (!Number.isInteger(tc) || tc < 1 || tc > 3)
        return NextResponse.json(
          { error: "term_count must be 1–3" },
          { status: 400 }
        );
      termNums = Array.from(
        { length: tc },
        (_, i) => ((resolvedStartTerm - 1 + i) % 3) + 1
      );
    }
  }
  ...
  const isOnlineChunk =
    format === "online" && Number.isInteger(moduleIdx) && moduleIdx >= 1;
  ...
  const hasExplicitSchool = Object.prototype.hasOwnProperty.call(
    body,
    "school_id"
  );
  ...
  if (hasExplicitSchool) {
    ...
    if (
      targetSchoolId !== null &&
      (typeof targetSchoolId !== "string" || !targetSchoolId.length)
    ) {
      return NextResponse.json(
        { error: "school_id must be a UUID, or null for platform template" },
        { status: 400 }
      );
    }
      ...
      if (format === "school" && existingCurriculumContent?.terms?.length) {
        ...
        if (resolvedYear > 1) {
          const priorYearTerms = (existingCurriculumContent.terms as any[])
            ...
            .sort(
              (a: any, b: any) =>
                (a.year ?? 1) - (b.year ?? 1) || a.term - b.term
            );
          if (priorYearTerms.length > 0) {
            const priorBlock = priorYearTerms
              .map((t: any) => {
                const yr = t.year ?? 1;
                const topics = (t.weeks ?? [])
                  ...
                  .map((w: any) => `    - ${w.topic}`)
                  .join("\n");
                return `Year ${yr}, Term ${t.term} (${
                  NG_TERM_LABEL[t.term] ?? ""
                }):\n${topics}`;
              })
              .join("\n");
            contextParts.push(
              `PRIOR YEAR(S) — all topics already taught across previous year(s):\n${priorBlock}`
            );
          }
        }
        ...
        const sameYearOtherTerms = (existingCurriculumContent.terms as any[])
          .filter(
            (t: any) =>
              (t.year ?? 1) === resolvedYear && !existingTermNums.has(t.term)
          )
          .sort((a: any, b: any) => a.term - b.term);
        if (sameYearOtherTerms.length > 0) {
          const sameYearBlock = sameYearOtherTerms
            .map((t: any) => {
              const topics = (t.weeks ?? [])
                .filter((w: any) => w.type === "lesson")
                .map((w: any) => `    - ${w.topic}`)
                .join("\n");
              return `Year ${resolvedYear}, Term ${t.term} (${
                NG_TERM_LABEL[t.term] ?? ""
              }):\n${topics}`;
            })
            .join("\n");
          contextParts.push(
            `SAME YEAR, OTHER TERMS already generated:\n${sameYearBlock}`
          );
        }
  ...
  const sourceMaterial =
    typeof body.source_material === "string"
      ? body.source_material.slice(0, 8000).trim()
      : "";
  ...
  const prompt = isOnlineChunk
    ? buildOnlineModulePrompt(
        ...
        Array.isArray(body.prior_module_themes)
          ? (body.prior_module_themes as unknown[])
  ...
  const aiContent = await generateCurriculum(
    prompt + CURRICULUM_UNIQUENESS_RULE
  );
  if (!aiContent) {
    return NextResponse.json(
      {
        error:
          "Syllabus generation failed — all AI models unavailable. Please try again.",
      },
      { status: 502 }
    );
  }
  ...
  if (isOnlineChunk && Array.isArray(aiContent.terms)) {
    aiContent.terms = aiContent.terms.map((t: any) => ({
      ...t,
      term: moduleIdx,
      year: 1,
    }));
    ...
    aiContent.terms = [...kept, ...aiContent.terms].sort(
    ...
    if (moduleIdx > 1 && existingCurriculumContent) {
      for (const k of [
        ...
        if (existingCurriculumContent[k] != null)
          aiContent[k] = existingCurriculumContent[k];
      }
    }
  }
  ...
  if (aiContent?.terms && Array.isArray(aiContent.terms)) {
    aiContent.terms = aiContent.terms.map((term: any, idx: number) => {
      const resolvedTerm =
        format === "school" && termNums[idx]
          ? termNums[idx]
          : term.term || resolvedStartTerm;
      return {
        ...term,
        term: resolvedTerm,
        year: term.year ?? resolvedYear,
        start_date:
          termStartDates[String(resolvedTerm)] ||
          termStartDates[resolvedTerm] ||
          term.start_date ||
          null,
      };
    });
  ...
  if (
    format === "school" &&
    existingCurriculumContent?.terms?.length &&
    aiContent?.terms
  ) {
    const existingTermNums = new Set(termNums);
    const keptTerms = (existingCurriculumContent.terms as any[]).filter(
      (t: any) =>
        (t.year ?? 1) !== resolvedYear || !existingTermNums.has(t.term)
    );
    const mergedTerms = [...keptTerms, ...aiContent.terms].sort(
      (a: any, b: any) => (a.year ?? 1) - (b.year ?? 1) || a.term - b.term
    );
...
export async function DELETE(req: NextRequest) {
  ...
  if (!profile || profile.role !== "admin") {
    return NextResponse.json(
      { error: "Only the Academic Office can delete curriculum drafts." },
      { status: 403 }
    );
  }
