...
export async function POST(
  ...
  if (source.school_id !== null) {
    return NextResponse.json(
      {
        error:
          "Only platform (shared) curricula can be cloned. School curricula can be edited directly.",
      },
      { status: 400 }
    );
  }
  ...
  const targetSchoolId: string | null =
    body.school_id ?? profile.school_id ?? null;

  if (!targetSchoolId) {
    return NextResponse.json(
      {
        error:
          "No school to clone to. Specify a school_id in the request body.",
      },
      { status: 400 }
    );
  }
  ...
  const write = existing
    ? admin
        ...
        .eq("id", existing.id)
    : admin.from("course_curricula").insert({
