  ...
  if (actor.role !== "admin") {
    return NextResponse.json({
      data: {
        is_admin: false,
        message:
          "The Academic Office manages official curriculum direction. Your class workspace shows what to teach and lets you customise delivery.",
      },
    });
  }
  const db: any = createAdminClient();
  const [{ data: curricula }, { data: editions }, { count: suggestions }] =
    await Promise.all([
      db
        .from("course_curricula")
        .select("id, course_id, updated_at, courses(title, programs(name))")
        .is("school_id", null)
...
export async function POST(req: NextRequest) {
  ...
  if (actor.role !== "admin") {
    return NextResponse.json(
      {
        error:
          "Only the Academic Office can publish an official academic direction.",
      },
      { status: 403 }
    );
  }
  ...
  const academicSession =
    typeof body.academic_session === "string"
      ...
      : "";
  const audienceLabel =
    typeof body.audience_label === "string" ? body.audience_label.trim() : "";
  const gradeKey =
    typeof body.grade_key === "string" ? body.grade_key.trim() : "";
  const effectiveTermNumber = [1, 2, 3].includes(
    Number(body.effective_term_number)
  )
    ? Number(body.effective_term_number)
    : 1;
  const changeSummary =
  ...
  if (!curriculumId || !academicSession || !audienceLabel || !changeSummary) {
    return NextResponse.json(
      {
        error:
          "Choose a curriculum and add the academic session, learner level, and a short explanation of what changed.",
      },
      { status: 400 }
    );
  }
  ...
  if (!curriculum || curriculum.school_id) {
    return NextResponse.json(
      { error: "Choose a central Academic Office curriculum draft." },
      { status: 404 }
    );
  }
  ...
  const title = `${course?.title ?? "Curriculum"} · ${humanAcademicSession(
    academicSession
  )} · ${level}`;
  ...
  return NextResponse.json(
    {
      data: {
        ...
        quality,
        assignment,
        message: `Official direction protected for ${humanTermLabel(
          effectiveTermNumber
        )} and assigned to ${assignment.applied_count} schools and ${
          assignment.offering_applied_count
        } unassigned programme pathways. ${
          assignment.independent_pathways_preserved
        } existing Online or Special pathway direction(s) were preserved. Existing class plans remain unchanged.`,
      },
    },
    { status: 201 }
  );
