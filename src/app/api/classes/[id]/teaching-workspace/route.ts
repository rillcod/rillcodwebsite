...
async function getActor(): Promise<Actor | null> {
  ...
  if (!user) return null;
  const { data } = await createAdminClient().from('portal_users').select('id,role,school_id').eq('id', user.id).maybeSingle();
...
async function scope(db: any, id: string, user: Actor) {
  ...
  if (user.role === 'teacher') {
    ...
    if (klass.teacher_id !== user.id || !ids.includes(klass.school_id)) return { error: 'This class is not assigned to you', status: 403 };
  }
  if (user.role === 'school' && klass.school_id !== user.school_id) return { error: 'This class belongs to another school', status: 403 };
  if (!['admin', 'teacher', 'school'].includes(user.role)) return { error: 'Staff access required', status: 403 };
  return { klass };
}
...
async function activeDirection(db: any, schoolId: string, courseId: string, preferredReleaseId?: string | null) {
  ...
  return relation<any>(adoption?.release);
}

async function directionSchedule(db: any, input: { schoolId: string; classId: string; courseId: string; releaseId: string }) {
  const { data } = await db.from('academic_curriculum_delivery_schedules').select('*')
    .eq('school_id', input.schoolId).eq('course_id', input.courseId).eq('release_id', input.releaseId).eq('status', 'active')
  ...
  return effectiveDeliverySchedule({
    ...
    classOverride: (data ?? []).find((row: any) => row.class_id === input.classId) ?? null,
  }) as any;
}

function mappedPlanWeeks(input: {
  content: any;
  ...
  const terms: any[] = Array.isArray(input.content?.terms) ? input.content.terms : [];
  for (let calendarWeek = 1; calendarWeek <= 12; calendarWeek += 1) {
    const position = mapSessionCalendarToCurriculumPosition({
      entryAcademicSession: input.academicSession,
      currentAcademicSession: input.currentSession,
    ...
    const term = terms.find((item) => Number(item.year ?? 1) === position.year && Number(item.term) === position.term)
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (scoped.error) return NextResponse.json({ error: scoped.error }, { status: scoped.status });
  const klass = scoped.klass;
  const courseId = new URL(req.url).searchParams.get('course_id') || klass.current_course_id || null;
  const { data: courses } = klass.program_id
    ? await db.from('courses').select('id,title,program_id,school_id').eq('program_id', klass.program_id).eq('is_active', true).order('level_order')
    : { data: [] };
  if (courseId && !(courses || []).some((course: any) => course.id === courseId)) {
    return NextResponse.json({ error: 'Course is not part of this class programme' }, { status: 400 });
  }
  ...
  if (courseId && klass.term_id) {
    ...
    if (plan) {
      const [lessonResult, deliveryResult, progressResult] = await Promise.all([
        ...
        db.from('class_lesson_delivery').select('*').eq('lesson_plan_id', plan.id).order('week_number'),
        db.from('class_term_teaching_progress').select('*').eq('lesson_plan_id', plan.id).maybeSingle(),
      ]);
  ...
  return NextResponse.json({
    data: {
      ...
      } : { available: false, message: 'The Academic Office has not assigned an official direction for this course yet.' },
      plan,
      lessons,
      deliveries,
      progress,
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await getActor();
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  if (!['admin', 'teacher'].includes(user.role)) return NextResponse.json({ error: 'Only teachers and administrators can change teaching records' }, { status: 403 });
  const { id } = await params;
  const db: any = createAdminClient();
  const scoped: any = await scope(db, id, user);
  if (scoped.error) return NextResponse.json({ error: scoped.error }, { status: scoped.status });
  ...
  if (body.action === 'ensure_plan') {
    if (!klass.term_id || !klass.academic_terms) return NextResponse.json({ error: 'Assign an academic term to this class first' }, { status: 400 });
    const courseId = typeof body.course_id === 'string' ? body.course_id : '';
    const { data: course } = await db.from('courses').select('id,program_id').eq('id', courseId).maybeSingle();
    if (!course || (klass.program_id && course.program_id && klass.program_id !== course.program_id)) {
      return NextResponse.json({ error: 'Choose a course from this class programme.' }, { status: 400 });
    }
    ...
    const schedule = await directionSchedule(db, { schoolId: klass.school_id, classId: id, courseId, releaseId: direction.id }) ?? {
      entry_term_number: direction.effective_term_number ?? klass.academic_terms.term_number ?? 1,
      entry_week_number: 1,
    ...
    const { data, error } = await db.rpc('ensure_class_term_teaching_plan', {
      ...
      p_actor_id: user.id,
      p_sessions_per_week: Number(body.sessions_per_week) || Number(schedule.sessions_per_week) || 1,
    });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    const result = data as { plan_id: string; created: boolean };
    if (!existing?.curriculum_release_id) {
      const weeks = mappedPlanWeeks({
        content: direction.content,
        academicSession: direction.academic_session,
        currentSession: klass.academic_terms.academic_year,
      ...
      await db.from('lesson_plans').update({
        ...
        plan_data: {
          academic_direction: {
            title: direction.title,
            academic_session: direction.academic_session,
            entry_point: humanEntryPoint({ termNumber: Number(schedule.entry_term_number), weekNumber: Number(schedule.entry_week_number) }),
            current_term: humanTermLabel(Number(klass.academic_terms.term_number)),
          },
    ...
    return NextResponse.json({ data: { ...result, official_direction: direction.title } }, { status: result.created ? 201 : 200 });
  }
  ...
  return NextResponse.json({ error: 'Unknown teaching action' }, { status: 400 });
}
