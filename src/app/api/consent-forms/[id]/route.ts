...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: responses, error: rErr }, { data: leads, error: lErr }] = await Promise.all([
    ...
    (supabase as any)
      .from('form_leads')
      .select('id, submitted_at, email, child_current_school, matched_school_id, response_data, schools!matched_school_id(name)')
      .eq('form_id', id)
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ['is_public', 'form_type'] as const;
