 ...
 *
 * Admin-only. Finds and repairs every inconsistency between the three user pools:
...
async function runAudit(admin: ReturnType<typeof adminClient>) {
  ...
  // 2. All portal_users
  const { data: portalRows } = await admin.from('portal_users').select('id, email, role, full_name');
  ...
  const { data: approvedStudents } = await admin
    .from('students')
    .select('id, full_name, student_email, parent_email, school_name, date_of_birth, user_id')
    .eq('status', 'approved')
    .is('user_id', null);
  ...
  // 4. Auth users with no portal_users row
  const authWithoutPortal = authUsers.filter(u => !portalIdSet.has(u.id));

  // 5. Portal users with no auth account
...
export async function GET() {
  ...
  const { orphanedApprovedStudents, authWithoutPortal, portalWithoutAuth } = await runAudit(admin);
...
export async function POST() {
  ...
  for (const s of orphanedApprovedStudents) {
    ...
    try {
      ...
      if (authErr) {
        // User already exists in auth — try to find them and link
        if (authErr.message.includes('already') ) {
          // Just try to create the portal_users row for the existing auth user
      ...
      // Upsert portal_users row (trigger may also create one — use ON CONFLICT)
      ...
      // Link student row back to the portal account
  ...
  // ── Gap 2: Auth users with no portal_users row ──────────────────────────
  ...
  // ── Gap 3: Portal users with no auth account — sync metadata to auth ────
  // (These are in portal_users but have no auth entry — we can't create auth
  //  without a password, so instead we just flag them and sync metadata for
  //  the ones that DO have matching auth entries by email)
  for (const pu of portalWithoutAuth as any[]) {
    try {
      ...
      if (matchingAuth && matchingAuth.id !== pu.id) {
        // There's an auth user with the same email but different ID — update portal_users to use the right ID
        await admin.from('portal_users').upsert({
          ...pu,
          id: matchingAuth.id,
          updated_at: new Date().toISOString(),
  ...
  return NextResponse.json({
    ...
    credentials: results.students_given_accounts,  // show admin the new passwords
