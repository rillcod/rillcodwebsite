...
export function resolveSyllabusPhaseFromClassName(className: string | null | undefined): 'ss_1_2' | 'jss_1_3' | 'basic_1_6' {
  const ssLevel = parseSsLevel(className);
  if (ssLevel && ssLevel <= 2) return 'ss_1_2';
...
export function resolveDefaultTrackFromPolicy(
  ...
  if (ssLevel && ssLevel <= 2) return ssTrack;
