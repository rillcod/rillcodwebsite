...
describe('reportCoverageForStudents', () => {
  it('scopes coverage to both term and academic period', async () => {
    ...
    const result = await reportCoverageForStudents(admin, ['student-1'], { termId: 'term-1', termLabel: 'First Term', periodLabel: '2026/2027' });
    expect(filters).toContainEqual(['term_id', 'term-1']);
    expect(filters).not.toContainEqual(['report_term', 'First Term']);
