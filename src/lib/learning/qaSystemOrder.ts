...
export const DELIVERY_MODE_CHOICES = [
  {
    ...
    body: "Use the Curriculum → Optional QA panel to preview/apply. Discard anytime and keep a freeform or AI syllabus.",
  },
  {
    id: "qa_spine_compulsory" as const,
    title: "Your standard: make spine compulsory",
    body: "Adopt the template as the house rule: set class `qa_*` and apply the spine to every new course_curricula for that scope; new migrations still update the catalog in one place (not scattered copies).",
  },
  {
    id: "traditional_week" as const,
    title: "Traditional week-by-week",
    body: "Ignore spine injection: build terms with /api/curricula generate or manual weeks, track on the same term/week grid without metadata.qa_spine.",
  },
];
...
export const LEARNING_QA_SYSTEM_ORDER: QaSystemLayer[] = [
  ...
  },
  {
    ...
    db: [
      {
        name: "public.platform_syllabus_week_template",
        note: "Read-only for teachers; seeded via migration",
      },
    ],
    ...
    codeRefs: [
      "src/lib/qa/rotatedSpineIndex.ts",
      "src/lib/catalog/platformSpineToSyllabusWeeks.ts",
    ],
  },
  {
    order: 3,
    ...
    db: [
      {
        name: "public.classes",
        note: "qa_spine_lane, qa_grade_key, qa_track_hint, …",
      },
    ],
    ...
    appPaths: [{ label: "Class edit (QA fields)", path: "/dashboard/classes" }],
    codeRefs: ["src/lib/qa/resolveQaSpineLane.ts"],
  },
  {
    order: 4,
    title: "Curriculum Studio draft",
    purpose:
      "The mutable authoring draft lives in `course_curricula`. Admins and AI can improve it, but classes, progression, and reports never treat an edited draft as the official runtime contract.",
    db: [
      {
        name: "public.course_curricula",
        note: "content JSON; metadata.qa_spine when spine applied",
      },
    ],
  ...
  {
    order: 5,
    title: "Official direction & term teaching plan",
    purpose:
    ...
    db: [
      {
        name: "lesson_plans.plan_data.progression",
        note: "generated_terms, y{n}t{n} week grids, term_status",
      },
      {
        name: "public.progression_override_audit",
        note: "term lock / override audit",
      },
    ...
    appPaths: [
      {
        label: "Lesson plans (link syllabus, then generate progression)",
        path: "/dashboard/lesson-plans",
      },
      {
        label: "Student progression (end of term / promote)",
        path: "/dashboard/learner-progress?view=decisions",
      },
      {
        label: "Progression audit log",
        path: "/dashboard/learner-progress?view=history",
      },
    ],
    codeRefs: [
      "src/lib/progression/termStatus.ts",
  ...
  },
  {
    ...
    db: [
      {
        name: "curriculum week tracking (see migration 20260501000050_curriculum_tracking.sql)",
      },
    ],
  ...
  },
  {
    ...
    db: [
      {
        name: "public.lesson_plans",
        note: "curriculum_version_id → course_curricula",
      },
      { name: "public.lessons" },
    ],
  ...
  },
  {
    ...
    purpose:
      "Content types generated from the same week topic — metadata often stores curriculum_id + week.",
    db: [
      { name: "public.assignments" },
      { name: "… (domain-specific tables)" },
    ],
    sqlMigrations: [
