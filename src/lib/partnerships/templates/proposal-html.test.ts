...
describe('the partnership proposal', () => {
  ...
  it('puts primary on one sheet and secondary on one sheet', () => {
    ...
    const primarySheets = html.split(/<div class="page[ "][^>]*>/).slice(1).filter((p) => p.includes('Primary Pathway'));
    ...
    const secondarySheets = html.split(/<div class="page[ "][^>]*>/).slice(1).filter((p) => p.includes('Secondary Pathway'));
...
describe('the money page', () => {
  it('does not highlight the last option as if it were the quote', () => {
    ...
    const returnPage = html.split(/<div class="page[ "][^>]*>/).slice(1).find((p) => p.includes("The school's share")) ?? '';
