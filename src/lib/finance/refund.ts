...
export async function finalizeFullRefund(
  ...
  if (!['completed', 'success', 'paid'].includes(String(tx.payment_status || '').toLowerCase())) return financeFail('invalid_transition', 'Only a completed payment can be refunded');
  ...
  const { data: refunded, error: refundError } = await db.from('payment_transactions').update({
    ...
    updated_at: now,
    payment_gateway_response: { ...existingGateway, refund: { ...input.gatewayRefund, reason, actor_id: input.actorId ?? null, finalized_at: now } },
  ...
  if (invoice?.id) {
    ...
    const nextStatus = nextPaid > 0 ? 'partially_paid' : (invoice.due_date && new Date(invoice.due_date).getTime() < Date.now() ? 'overdue' : 'sent');
    ...
    if (invoiceError) return financeFail('db_error', 'Payment refunded but invoice reversal failed: ' + invoiceError.message, undefined, { transaction_id: tx.id });
    const rosterInvoice = await syncRosterBillingForInvoice(db as any, invoice.id, nextStatus);
    if (!rosterInvoice.ok) return financeFail('db_error', 'Invoice reversed but roster sync failed: ' + rosterInvoice.error, undefined, { transaction_id: tx.id, invoice_id: invoice.id });
    ...
    if (invoice.billing_cycle_id) {
      ...
      const { error: cycleError } = await db.from('billing_cycles').update({ status: cycleStatus, updated_at: now }).eq('id', invoice.billing_cycle_id).eq('status', 'paid');
      if (cycleError) return financeFail('db_error', 'Invoice reversed but billing cycle reopening failed: ' + cycleError.message, undefined, { transaction_id: tx.id });
      const rosterCycle = await syncRosterBillingForCycle(db as any, invoice.billing_cycle_id, cycleStatus);
      if (!rosterCycle.ok) return financeFail('db_error', 'Billing cycle reopened but roster sync failed: ' + rosterCycle.error, undefined, { transaction_id: tx.id });
  ...
  if (tx.course_id && tx.portal_user_id) {
    ...
    if (courseError) return financeFail('db_error', 'Refund completed but course lookup failed: ' + courseError.message, undefined, { transaction_id: tx.id });
    if (course?.program_id) {
      const { error: enrollmentError } = await db.from('enrollments').update({ status: 'suspended' }).eq('user_id', tx.portal_user_id).eq('program_id', course.program_id);
      if (enrollmentError) return financeFail('db_error', 'Refund completed but enrolment suspension failed: ' + enrollmentError.message, undefined, { transaction_id: tx.id });
