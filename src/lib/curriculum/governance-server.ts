...
export async function requireGovernanceActor(): Promise<GovernanceActor | null> {
  ...
  const { data } = await db
    ...
    .eq("id", user.id)
    .maybeSingle();
  if (!data || !["admin", "teacher", "school"].includes(data.role ?? ""))
    return null;
  return data as GovernanceActor;
}

export async function actorSchoolIds(
  actor: GovernanceActor
): Promise<string[]> {
  if (actor.role === "admin") return [];
  if (actor.role === "teacher")
    return getTeacherSchoolIds(actor.id, actor.school_id);
...
export async function previewCurriculumRollout(input: {
  ...
  const { data: release, error } = await db
    ...
    .select(
      "id, course_id, release_number, title, status, academic_session, effective_term_number, courses(program_id, title)"
    )
    .eq("id", input.releaseId)
    .maybeSingle();
  if (error || !release)
    throw new Error("Published curriculum release not found.");
  if (release.status !== "published")
    throw new Error("Only published releases can be rolled out.");

  const course = Array.isArray(release.courses)
    ? release.courses[0]
    : release.courses;
  ...
  for (const school of allSchools ?? [])
    if (school.id) schoolSet.add(school.id);
  if (programId) {
    ...
    for (const row of classes ?? [])
      if (row.school_id) schoolSet.add(row.school_id);
  }
  ...
  for (const row of existingCurricula ?? [])
    if (row.school_id) schoolSet.add(row.school_id);
  const { data: existingAdoptions } = await db
    .from("academic_curriculum_adoptions")
    .select("id, school_id, release_id, auto_update, status")
    .eq("course_id", release.course_id)
    .eq("academic_session", release.academic_session)
    .eq("effective_term_number", release.effective_term_number);
  for (const row of existingAdoptions ?? [])
    if (row.school_id) schoolSet.add(row.school_id);
  ...
  for (const plan of activePlans ?? []) {
    if (plan.school_id)
      planCount.set(plan.school_id, (planCount.get(plan.school_id) ?? 0) + 1);
  }
  const adoptionMap = new Map(
    (existingAdoptions ?? []).map((row: any) => [row.school_id, row])
  );
  const nameMap = new Map<string, string>(
    (schools ?? []).map((row: any) => [
      String(row.id),
      String(row.name ?? "Unnamed school"),
    ])
  );

  const rows: RolloutSchool[] = schoolIds
    .map((schoolId) => {
  ...
  return {
    ...
    summary: {
      ...
      protected_active_plans: rows.reduce(
        (sum, row) => sum + row.active_plan_count,
        0
      ),
    },
  };
...
export async function applyCurriculumRollout(input: {
  ...
  const release = preview.release as {
    id: string;
    course_id: string;
  ...
  for (const school of preview.schools) {
    ...
    const payload = {
      ...
      academic_session: release.academic_session,
      previous_release_id: school.current_release_id,
      effective_term_number: release.effective_term_number,
      status: "active",
      auto_update: true,
    ...
    const { data: adoption, error } = await db
      .from("academic_curriculum_adoptions")
      .upsert(payload, { onConflict: "school_id,course_id,academic_session,effective_term_number" })
      .select("id")
      .single();
  ...
  if (programmeId) {
    ...
    const offeringIds = (offerings ?? [])
      ...
      .filter(Boolean);
    const { data: currentDirections, error: directionError } =
      offeringIds.length
        ? await db
    ...
    const directedOfferingIds = new Set(
      (currentDirections ?? []).map(
        (direction: any) => direction.academic_offering_id
      )
    );

    for (const offering of offerings ?? []) {
      const pathway = String(
        offering.pathway ?? offering.enrollment_type ?? "school_term"
      );
      ...
      offeringAppliedByPathway[pathway] =
