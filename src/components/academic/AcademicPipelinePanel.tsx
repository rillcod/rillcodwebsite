...
export function AcademicPipelinePanel({
  ...
  return (
    <section
      className={
        embedded ? 'p-3' : 'rounded-3xl border border-border bg-card p-6'
      }
    >
      {!embedded && (
      ...
      {embedded && (
        <div className="mb-2 flex justify-end">
          <button
            ...
            onClick={() => void load()}
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-bold hover:bg-muted disabled:opacity-50"
          >
            <ArrowPathIcon
              ...
              <ul className="mt-2 space-y-1.5">
                {data.coverage.map((row) => (
                  ...
                  >
                    ...
                    >
                      {row.publishedCount} of {row.courseCount} course
                      {row.courseCount === 1 ? '' : 's'}
                      {row.publishedCourses.length
                        ? ` · ${row.publishedCourses.join(', ')}`
                        : ' · nothing published'}
                    </span>
                  </li>
                ))}
          ...
          {data.resolving.length > 0 && (
            <details className="mt-4 rounded-xl border border-border bg-background p-3">
              <summary className="cursor-pointer text-sm font-bold text-foreground">
                {data.resolving.length} class
                {data.resolving.length === 1 ? '' : 'es'} will be set
                automatically
              </summary>
              <ul className="mt-2 max-h-40 space-y-0.5 overflow-y-auto text-xs text-muted-foreground">
                {data.resolving.map((c) => (
          ...
          {data.blocked.length > 0 && (
            <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
              <p className="flex items-center gap-2 text-sm font-black text-amber-700 dark:text-amber-300">
                ...
                {data.blocked.length} class
                {data.blocked.length === 1 ? '' : 'es'} need a decision
              </p>
                ...
                {(data.blockedByProgramme ?? []).map((group) => (
                  ...
                  >
                    <p className="font-black text-foreground">
                      {group.programme} — {group.classCount} class
                      {group.classCount === 1 ? '' : 'es'} waiting
                    </p>
                    <p className="mt-0.5 text-muted-foreground">
                      {group.courseCount === 0
                        ? 'This programme has no courses yet.'
                        : group.publishedCount === 0
                          ? `${group.courseCount} course${group.courseCount === 1 ? '' : 's'}, none published.`
                          : `${group.publishedCount} of ${group.courseCount} published — someone must choose.`}
                    </p>
            ...
            {Object.values(data.jobs).map((job) => (
              ...
              >
                <p className="text-xs font-black text-foreground">
                  {JOB_LABEL[job.job_name] ?? job.job_name}
                </p>
                <p className="mt-1 text-[11px] text-muted-foreground">
                  Last run {when(job.last_finished_at)} · last success{' '}
                  {when(job.last_success_at)}
                </p>
                {job.consecutive_failures > 0 && (
                  <p className="mt-1 text-[11px] font-bold text-rose-500">
                    {job.consecutive_failures} failure
                    {job.consecutive_failures === 1 ? '' : 's'} in a row
