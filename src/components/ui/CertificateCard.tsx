...
type TemplateType = 'prestige' | 'royal' | 'tech' | 'scholar' | 'elite' | 'spark';
        ...
        <>
            ...
            {template === 'scholar' && (
                <div className="absolute inset-0" style={{ background: 'linear-gradient(160deg, #f0f4ff 0%, #e8eeff 60%, #f0f4ff 100%)' }}>
                    ...
                    {/* Subtle watermark */}
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.04, pointerEvents: 'none' }}>
                        <p style={{ fontSize: 120, fontWeight: 900, color: '#1e3a5f', textTransform: 'uppercase', letterSpacing: '-0.05em', userSelect: 'none' }}>RC</p>
                    </div>
            ...
            {template === 'spark' && (
                <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #0d0221 0%, #11083a 40%, #0a1628 100%)' }}>
                    {/* Animated-looking gradient orbs */}
...
export function CertificateCard({ cert }: CertificateProps) {
    ...
    const handleDownload = async () => {
        ...
        // Bring the capture div into the viewport so html-to-image renders it
        // fully — elements far off-screen can be skipped by the browser's GPU
        // rasterizer resulting in a blank canvas.
        setIsCapturing(true);
    ...
    return (
        <div className="space-y-5 animate-fade-in">
            ...
            {/* Dark screen overlay while capturing — prevents flash */}
            {isCapturing && (
            ...
            {/* Hidden fixed-size capture div — briefly moved into viewport on download */}
            <div
                ref={captureRef}
                aria-hidden="true"
                style={{
