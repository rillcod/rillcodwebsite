...
export default function BalanceRemindersPanel({
  ...
  async function act(action: string, prospectId?: string) {
    ...
    try {
      ...
      if (!r.ok || j.ok === false) {
        throw new Error(j.error || j.result?.error || 'Balance reminder action did not complete');
      }
      if (action === 'run_now') setRunResult(j.result ?? j);
      await load();
    } catch (error) {
      setRunResult({ error: error instanceof Error ? error.message : 'Balance reminder action failed' });
    } finally {
      setBusy(null);
    }
  }
