  ...
  return (
    ...
    >
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-1.5 flex items-center gap-3">
        <EyeIcon className="w-4 h-4 text-amber-700 dark:text-amber-300 flex-shrink-0" />
        <p className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.22em] text-amber-800 dark:text-amber-200 truncate">
          Previewing as <span className="text-amber-800 dark:text-amber-100">{ROLE_LABEL[viewAsRole] ?? viewAsRole}</span>
          <span className="hidden sm:inline text-amber-800/70 dark:text-amber-200/70 font-semibold normal-case tracking-normal ml-2">
            · Server enforces your real role ({ROLE_LABEL[actualRole ?? ''] ?? actualRole})
          </span>
        </p>
        <button
          onClick={() => setViewAsRole(null)}
          className="ml-auto flex items-center gap-1 px-2.5 py-1 bg-amber-500/25 hover:bg-amber-500/40 border border-amber-500/40 text-amber-800 dark:text-amber-100 text-[10px] font-black uppercase tracking-[0.2em] transition"
        >
