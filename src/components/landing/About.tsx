...
import { Target, Lightbulb, Award, Command } from 'lucide-react';
...
const About: React.FC = () => {
  return (
    <section id="about" className="py-24 bg-background relative overflow-hidden font-sans">
      {/* Background Decor */}
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              {pillars.map((p) => (
                <div key={p.title} className="group p-6 bg-card border border-border rounded-none hover:bg-muted transition-all border-b-2 border-b-transparent hover:border-b-brand-red-600 shadow-sm">
                  ...
                  <p className="text-[11px] text-muted-foreground font-bold leading-relaxed">{p.desc}</p>
                </div>
              ))}
            </div>
          </div>
