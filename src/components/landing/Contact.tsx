          ...
          <div className="lg:col-span-4 space-y-4">
            {contactCards.map((c, i) => (
              <div key={i} className="group flex items-center gap-5 bg-card border border-border rounded-none p-6 hover:bg-muted transition-all border-l-2 border-l-transparent hover:border-l-orange-500 shadow-sm">
                 ...
                 <div>
                    <h4 className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1">{c.title}</h4>
                    <p className="text-sm font-black text-foreground tracking-tight">{c.value}</p>
                    <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest mt-0.5">{c.sub}</p>
                 </div>
              </div>
            ))}
