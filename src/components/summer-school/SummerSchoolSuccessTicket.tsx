...
export function SummerSchoolSuccessTicket({
  ...
  learnerAge,
  learnerGrade,
  programmeTitle = "Rillcod special programme",
}: Props) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="printable-ticket-box">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
              Thank you for enrolling {successInfo.studentName} in {programmeTitle}. Kids, teens, adults, and individual learners are welcome at Rillcod.
            </p>
          </div>
        </div>
        ...
        <div className="border border-dashed border-border bg-background p-5 sm:p-6 rounded-xl space-y-4 relative text-left mt-6 sm:mt-8">
          <div className="flex justify-between items-start border-b border-border pb-3 sm:pb-4">
            <div>
              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Learner</p>
      ...
      <div className="space-y-3">
        ...
        {!compact && (
          <div className="flex items-start gap-3 p-4 bg-primary/5 border border-primary/20 rounded-xl text-left no-print">
            <span className="text-xl">🗺️</span>
            <div>
              <p className="text-xs font-black text-foreground uppercase tracking-wide">{AFTER_COHORT_BANNER.heading}</p>
              <p className="text-[10px] text-muted-foreground leading-relaxed mt-1">
                Suggested next programme: <span className="font-bold text-foreground">{next.programme}</span>. {next.note}
              </p>
              <Link href="/student-registration" className="inline-block mt-2 text-[10px] font-black uppercase tracking-widest text-primary hover:underline">
                Continue into term enrolment →
              </Link>
              <Link href="/programs" className="inline-block mt-2 ml-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground">
