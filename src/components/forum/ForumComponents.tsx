...
export function DiscussionTopic({ topic }: TopicProps) {
    return (
        <Card className={`group border-none shadow-md hover:shadow-xl transition-all duration-300 rounded-2xl overflow-hidden ${topic.is_pinned ? 'ring-2 ring-teal-500/20' : ''}`}>
            <CardHeader className="bg-card dark:bg-slate-900 border-b border-slate-50 dark:border-slate-800 p-6">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-bold text-foreground dark:text-slate-100">{topic.author_name}</p>
            ...
            <CardContent className="p-6 bg-card dark:bg-slate-900 text-muted-foreground dark:text-muted-foreground/70 text-sm leading-relaxed">
...
export function DiscussionReply({ reply, isAccepted = false }: { reply: any, isAccepted?: boolean }) {
    return (
        <div className={`relative pl-4 ml-2 border-l-2 transition-colors ${isAccepted ? 'border-green-500/30' : 'border-slate-100 dark:border-slate-800'}`}>
            <Card className={`border-none ${isAccepted ? 'bg-green-50/30 ring-1 ring-green-500/20' : 'bg-card dark:bg-slate-900'} shadow-sm`}>
