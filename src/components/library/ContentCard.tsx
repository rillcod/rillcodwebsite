...
export function ContentLibraryCard({ item }: { item: ContentItem }) {
    ...
    return (
        <Card className="group overflow-hidden border-none shadow-lg hover:shadow-2xl transition-all duration-500 rounded-3xl bg-white dark:bg-slate-900">
            ...
            <div className="relative aspect-video bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                    <Badge className="bg-white/90 dark:bg-slate-900/90 text-slate-900 dark:text-white backdrop-blur uppercase text-[9px] font-black px-3 py-1 rounded-full border-none">
                        {item.content_type}
                    </Badge>
                </div>
            ...
            <CardContent className="p-6 space-y-4">
                <div className="space-y-1">
                    <div className="flex justify-between items-start gap-4">
                        <h4 className="font-bold text-slate-800 dark:text-slate-100 line-clamp-1 leading-tight">{item.title}</h4>
                        <div className="flex items-center gap-1 shrink-0">
                            <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                            <span className="text-xs font-bold text-slate-500">{item.average_rating || 'N/A'}</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{item.created_by_name}</span>
                    </div>
                </div>

                <div className="flex flex-wrap gap-1">
                    {item.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[9px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded uppercase">#{tag}</span>
                    ))}
                </div>
            </CardContent>

            <CardFooter className="px-6 py-4 bg-slate-50/50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
